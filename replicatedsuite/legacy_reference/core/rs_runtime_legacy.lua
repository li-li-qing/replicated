...
function R:Stop()
    ...
    if S.Persistence and type(S.Persistence.Flush) == "function" then pcall(function() S.Persistence:Flush() end) end
    ...
    if S.UIHostManager and type(S.UIHostManager.HideAll) == "function" then
...
function R:InstallSchedulerTasks()
    ...
    S.Scheduler:AddTask("ui_flush",S.Constants.Refresh.uiFlushMs,function()
        if S.State:HasDirty() then
            local dirty = S.State:ConsumeDirty()
            if S.UIHostManager ~= nil and type(S.UIHostManager.RefreshData) == "function" then
                S.UIHostManager:RefreshData(dirty)
            end
        end
    end,false,nil,"P4")
...
function R:RegisterPresentationHosts()
    ...
    if manager:IsRegistered("legacy") ~= true then
        return false, "legacy presentation host adapter not registered"
    end
    -- Legacy must remain available during migration, but Runtime no longer
    -- forces it to be the default. A future V3 host can explicitly become the
    -- default without another Core rewrite.
    local defaultId = tostring(manager.defaultId or "legacy")
    if manager:IsRegistered(defaultId) ~= true then defaultId = "legacy" end
    manager.defaultId = defaultId
    manager.activeId = manager.activeId or defaultId
    return true
end
...
function R:RegisterEscMenu()
    ...
    local contentOk, contentErr = ReplicatedEscMenuPolicy:RegisterContent(SUITE_CONTENT_ID, window, function(show)
        ...
        if desiredVisible ~= currentVisible then
            if type(manager.Toggle) == "function" then manager:Toggle() end
...
function R:Start()
    ...
    local success, failure = xpcall(function()
        ...
        if S.GameDataRegistry ~= nil and type(S.GameDataRegistry.Validate) == "function" then
            ...
            if report.ok ~= true and S.DiagnosticsManager ~= nil and type(S.DiagnosticsManager.Warn) == "function" then
                S.DiagnosticsManager:Warn("game_data", "REGISTRY_VALIDATION_ISSUES",
                    "共享游戏数据注册表存在校验问题", {
                        errors = tonumber(report.errors) or 0,
                        warnings = tonumber(report.warnings) or 0,
                        records = tonumber(report.totalRecords) or 0,
                        sets = tonumber(report.totalSets) or 0,
                    })
            elseif S.DiagnosticsManager ~= nil and type(S.DiagnosticsManager.Count) == "function" then
                S.DiagnosticsManager:Count("game_data", "REGISTRY_VALIDATION_OK", 1)
            end
        end
        if S.StaticDataV2 ~= nil and type(S.StaticDataV2.Validate) == "function" then
            local report = S.StaticDataV2:Validate()
            if report.ok ~= true and S.DiagnosticsManager ~= nil and type(S.DiagnosticsManager.Warn) == "function" then
                S.DiagnosticsManager:Warn("static_data_v2", "VALIDATION_ISSUES", "Static Data V2 存在校验问题，V3 Foundation 将保持 BLOCKED", {
                    errors = #(report.errors or {}),
                    warnings = #(report.warnings or {}),
                    records = tonumber(report.records) or 0,
                    references = tonumber(report.references) or 0,
                })
            elseif S.DiagnosticsManager ~= nil and type(S.DiagnosticsManager.Count) == "function" then
                S.DiagnosticsManager:Count("static_data_v2", "VALIDATION_OK", 1)
            end
        end

        -- All static game-definition imports have completed before Runtime.lua
        -- executes (TOC order). Seal both identity and relationship registries so
        -- Services/UI cannot quietly create new magic IDs during live play.
        EnterStage("game_data_seal")
        if S.GameDataRegistry ~= nil and type(S.GameDataRegistry.Seal) == "function" then
            S.GameDataRegistry:Seal("runtime_start")
        end
        if S.StaticDataV2 ~= nil and type(S.StaticDataV2.Seal) == "function" then
            S.StaticDataV2:Seal("runtime_start")
        end
        ...
        local hostOk, hostErr = R:RegisterPresentationHosts()
        if hostOk ~= true then error("presentation host registration: " .. tostring(hostErr or "failed")) end
        local defaultHostId = tostring(S.UIHostManager.defaultId or "legacy")
        local host = S.UIHostManager:Ensure(defaultHostId)
        if host == nil then error("default presentation host create failed: " .. defaultHostId) end
        S.UIHostManager.activeId = defaultHostId
        ...
        S.State:MarkDirty("all")
        local initialDirty = S.State:ConsumeDirty()
        if S.UIHostManager == nil or type(S.UIHostManager.RefreshData) ~= "function" then error("presentation refresh authority unavailable") end
        S.UIHostManager:RefreshData(initialDirty)

        EnterStage("bootstrap_warmup")
        R:InstallBootstrapDataWarmup()

        EnterStage("layout_finalize")
        if S.UIHostManager == nil or type(S.UIHostManager.ApplyResponsiveLayout) ~= "function" then error("presentation layout authority unavailable") end
        S.UIHostManager:ApplyResponsiveLayout(false)
    ...
    if not success then
        ...
        if S.Events then pcall(function() S.Events:Stop() end) end
        if S.Scheduler then pcall(function() S.Scheduler:Stop() end) end
        if S.UIHostManager then pcall(function() S.UIHostManager:HideAll(true) end) end
