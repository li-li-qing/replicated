...
function SettingWindowSkin(window)
	...
	if anchorValue == invalidInset then
		-- Addon builds can use freely resizable windows whose widths are not one of
		-- the stock skin presets.  X2Util:RaiseLuaCallStack is not on the public
		-- addon allow-list, so choose the nearest safe inset locally instead of
		-- calling the blocked diagnostic API.
		if width < 390 then
			anchorValue = 4
	...
	local upperCoords =
		UIParent:GetTextureData("ui/common/default.dds", string.format("bg_top_%s", coordSize))["coords"]
	local lowerCoords_l =
		UIParent:GetTextureData("ui/common/default.dds", string.format("bg_bottom_%s_left", coordSize))["coords"]
	local lowerCoords_r =
		UIParent:GetTextureData("ui/common/default.dds", string.format("bg_bottom_%s_right", coordSize))["coords"]
