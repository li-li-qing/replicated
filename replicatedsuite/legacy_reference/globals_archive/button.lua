...
local SIMPLE_BUTTON_LAYOUT_VERSION = "grid_v4_"
local SIMPLE_BUTTONS_PER_ROW = 4
local SIMPLE_BUTTON_WIDTH = 80
local SIMPLE_BUTTON_HEIGHT = 25
local SIMPLE_BUTTON_GAP_X = 10
local SIMPLE_BUTTON_GAP_Y = 7
local simpleButtonLayoutIndex = 0

local function GetButtonSaveKey(buttonText)
	return SIMPLE_BUTTON_LAYOUT_VERSION .. tostring(buttonText)
end

local function SaveButtonPosition(buttonText, x, y)
	local buttonSettings = { ["x"] = x, ["y"] = y }
	local buttonSaveKey = GetButtonSaveKey(buttonText)
	ADDON:ClearData(buttonSaveKey)
	ADDON:SaveData(buttonSaveKey, buttonSettings)
end

local function LoadSavedPosition(buttonText)
	local buttonSettings = ADDON:LoadData(GetButtonSaveKey(buttonText))
...
local function GetDefaultGridPosition(buttonIndex)
	local screenW = 1280
	local screenH = 720
	if UIParent ~= nil then
		if UIParent.GetScreenWidth ~= nil then
			screenW = UIParent:GetScreenWidth() or screenW
		end
		if UIParent.GetScreenHeight ~= nil then
			screenH = UIParent:GetScreenHeight() or screenH
		end
	end

	local startX = math.floor(screenW / 4)
	local startY = math.floor(screenH / 4)
	local column = buttonIndex % SIMPLE_BUTTONS_PER_ROW
	local row = math.floor(buttonIndex / SIMPLE_BUTTONS_PER_ROW)
	local x = startX + column * (SIMPLE_BUTTON_WIDTH + SIMPLE_BUTTON_GAP_X)
	local y = startY + row * (SIMPLE_BUTTON_HEIGHT + SIMPLE_BUTTON_GAP_Y)
...
function CreateSimpleButton(buttonText, x, y)
	...
	local defaultX, defaultY = GetDefaultGridPosition(simpleButtonLayoutIndex)
	simpleButtonLayoutIndex = simpleButtonLayoutIndex + 1
	if savedX ~= nil and savedY ~= nil then
		newButton:AddAnchor("TOPLEFT", "UIParent", savedX, savedY)
	else
		newButton:AddAnchor("TOPLEFT", "UIParent", defaultX, defaultY)
	end
	newButton:Show(true)
