...
F.presentationBoundary = "service_only"
F.presentationDebt = nil
...
local RECOVERY_STORE_ID = "suite.fishing_hotkey_recovery"
local READ_OPTIONS = { false, true, 0, 1 }

F.recoveryState = { pending = false }
local Persistence = S.Persistence
if type(Persistence) ~= "table" or type(Persistence.RegisterStore) ~= "function" then
    S.BootError = "fishing persistence unavailable"
    return
end
local recoveryStore, recoveryStoreErr = Persistence:RegisterStore({
    id = RECOVERY_STORE_ID,
    owner = "Fishing",
    contractVersion = 2,
    lifetime = Persistence.Lifetime.Checkpoint,
    scope = Persistence.Scope.Account,
    key = RECOVERY_KEY,
    schemaVersion = 1,
    legacySchemaVersion = 0,
    budget = { maxDepth = 10, maxNodes = 512, maxStringBytes = 8192, maxEntriesPerTable = 64 },
    default = function() return { pending = false } end,
    get = function() return F.recoveryState end,
    apply = function(value)
        F.recoveryState = type(value) == "table" and value or { pending = false }
    end,
})
if recoveryStore == nil then
    S.BootError = "fishing persistence register: " .. tostring(recoveryStoreErr or "unknown")
    return
end
...
function F:WriteRecovery(snapshot)
    ...
    self.recoveryState = payload
    local ok = Persistence:SaveValue(RECOVERY_STORE_ID, payload, { consumeDirty = true })
    return ok == true
end

function F:ClearRecovery()
    self.recoveryState = { pending = false }
    local ok = Persistence:SaveValue(RECOVERY_STORE_ID, self.recoveryState, { consumeDirty = true })
    return ok == true
end

function F:LoadRecovery()
    local store = Persistence:GetStore(RECOVERY_STORE_ID)
    if store ~= nil and store.loaded ~= true and store.loadStatus ~= "empty" then
        Persistence:LoadStore(RECOVERY_STORE_ID, { apply = true })
    end
    local value = self.recoveryState
