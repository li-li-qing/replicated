...
U.presentationBoundary = "service_only"
U.presentationDebt = nil
...
-- Manual raid checks are explicit user actions.  Persist only their small,
-- session-local result projection so the RSUI workspace can render the last
-- scan without re-reading any X2 team/buff APIs on its 1s presentation timer.
local function PublishManualCheck(kind, payload)
    kind = tostring(kind or "")
    if kind == "" then return end
    U.manualChecks = type(U.manualChecks) == "table" and U.manualChecks or {}
    U.manualCheckRevision = (tonumber(U.manualCheckRevision) or 0) + 1
    payload = type(payload) == "table" and payload or {}
    payload.kind = kind
    payload.revision = U.manualCheckRevision
    payload.checkedAt = tonumber(payload.checkedAt) or (S.NowMs ~= nil and S.NowMs() or 0)
    U.manualChecks[kind] = payload
    SetTeamData({
        manualCheckRevision = U.manualCheckRevision,
        manualCheckKind = kind,
        manualCheckAt = payload.checkedAt,
    })
end

local function CopyCounts(source)
    local out = {}
    for key, value in pairs(type(source) == "table" and source or {}) do out[key] = tonumber(value) or 0 end
    return out
end

local function CopyRows(source, limit)
    local out = {}
    limit = math.max(0, math.floor(tonumber(limit) or 100))
    for index, row in ipairs(type(source) == "table" and source or {}) do
        if limit > 0 and #out >= limit then break end
        local copy = {}
        for key, value in pairs(type(row) == "table" and row or {}) do
...
function U:SetPresenter(presenter)
...
function U:_Present(method, ...)
    ...
    if not ok then
        if S.DiagnosticsManager ~= nil and type(S.DiagnosticsManager.ErrorRateLimited) == "function" then
            S.DiagnosticsManager:ErrorRateLimited("team_utility", "PRESENTER_CALL_FAILED", 3000,
...
function U:HideOverlay(unitId)
    self:_Present("HideOverlay", unitId)
end

function U:HideAllSacOverlays()
    self:_Present("HideAll")
    self.activeSac = {}
...
function U:ScanSacBuffs()
    ...
    for unitId, candidate in pairs(self.candidates) do
        ...
        if buffId ~= nil then
            ...
            self:_Present("ShowOverlay", unitId)
        end
    end
...
function U:UpdateSacPositions()
    ...
    for unitId, active in pairs(self.activeSac) do
        ...
        if not ok or tonumber(x) == nil or tonumber(y) == nil or tonumber(z) == nil then
            ...
            self:_Present("UpdateOverlay", unitId, tonumber(x), tonumber(y), remaining)
        end
    end
end
...
function U:RunBuffCheck()
    ...
    local checked = ForEachOwnRaidMember(function(unitId, name, memberIndex, teamIndex)
        -- Role stats depend only on GetRole and always run: the button is
        -- "Buff / 职业检查"; unavailable Buff reads must not swallow the
        ...
        local row = {
            slot = FormatSlot(teamIndex, memberIndex),
            name = tostring(name or "?"),
            role = tostring(RAID_ROLE_LABELS[kind] or kind),
            roleKind = kind,
            state = canCount and canBuff and "读取中" or "Buff API不可用",
            missing = {},
        }
        memberRows[#memberRows + 1] = row
        if canCount and canBuff then
            local okCount, count = pcall(function() return X2Unit:UnitBuffCount(unitId) end)
            local buffCount = okCount and math.max(0, tonumber(count) or 0) or 0
            if buffCount == 0 then
                unreadable[#unreadable + 1] = string.format("%s (%s)", name, row.slot)
                row.state = "无法读取"
                row.unreadable = true
                return
            end
            ...
            for category, present in pairs(has) do
                if present ~= true then
                    row.missing[#row.missing + 1] = category
                    missingByCategory[category][#missingByCategory[category] + 1] = string.format("%s (%s)", name, row.slot)
                end
            end
            table.sort(row.missing)
            row.state = #row.missing == 0 and "完整" or ("缺 " .. table.concat(row.missing, ","))
        end
    end)
    ...
    local result = {
        ok = checked > 0,
        checked = checked,
        roleCounts = CopyCounts(roleCounts),
        rows = memberRows,
        unreadableCount = #unreadable,
        buffReadable = canCount and canBuff,
        message = checked == 0 and "未读取到团队成员" or nil,
    }
    PublishManualCheck("buff", result)
    ...
    if not anyMissing and #unreadable == 0 then S.SafeChat("所有可读成员均具备已检查的 Buff。") end
    return true, nil
end
...
function U:RunSiegeCheck()
    ...
    if not canCount or not canBuff then
        PublishManualCheck("siege", {
            ok = false, checked = 0, rows = {}, hiddenReadable = false,
            message = "Hidden Buff 能力不可用",
        })
    ...
    local checked = ForEachOwnRaidMember(function(unitId, name, memberIndex, teamIndex)
                    ...
                    for category, ids in pairs(RAID_SIEGE_BUFFS) do
                        for _, id in ipairs(ids) do
                            if tonumber(id) == buffId then
                                detected[category][#detected[category] + 1] = string.format("%s (%s)", name, row.slot)
                                row.detected[#row.detected + 1] = category
                            end
                        end
                    end
                end
        ...
        table.sort(row.detected)
        if #row.detected > 0 then row.state = table.concat(row.detected, ",") end
    end)
    if checked == 0 then
        PublishManualCheck("siege", {
            ok = false, checked = 0, rows = {}, hiddenReadable = true,
            message = "未读取到团队成员",
        })
        return false, "未读取到团队成员"
    end
    PublishManualCheck("siege", {
        ok = true,
        checked = checked,
    ...
    if not anyDetected then S.SafeChat("当前可读取成员中未检测到攻城装备状态。") end
    if #unreadable > 0 then S.SafeChat(string.format("无法读取（可能超出范围）：%s", table.concat(unreadable, "、"))) end
    return true, nil
end

-- Presentation-only snapshots for the M5 v6 Team Workspace.  These methods
-- never perform Native reads; they expose only already-committed Service state
-- and the results of explicit user-triggered checks.
function U:GetWorkspaceOverview()
    local settings = S.State ~= nil and type(S.State.settings) == "table" and S.State.settings or {}
    local data = S.State ~= nil and type(S.State.data) == "table" and type(S.State.data.teamUtility) == "table"
        and S.State.data.teamUtility or {}
    local caps = {}
    for key, value in pairs(type(self.capabilities) == "table" and self.capabilities or {}) do caps[key] = value == true end
    local active = 0
    for _ in pairs(type(self.activeSac) == "table" and self.activeSac or {}) do active = active + 1 end
    local candidates = 0
    for _ in pairs(type(self.candidates) == "table" and self.candidates or {}) do candidates = candidates + 1 end
    return {
        started = self.started == true,
        role = {
            enabled = settings.teamAutoRoleEnabled == true,
            mode = tostring(settings.teamRoleMode or "auto"),
            modeLabel = self:GetRoleModeLabel(),
            status = tostring(data.roleStatus or "--"),
            label = tostring(data.roleLabel or "--"),
            source = tostring(data.roleSource or data.lastRoleReason or "--"),
            classKey = tostring(data.classKey or "--"),
        },
        sacrifice = {
            enabled = settings.sacMarkerEnabled == true,
            candidates = tonumber(data.sacCandidates) or candidates,
            active = tonumber(data.sacActive) or active,
        },
        marker = {
            available = data.markerScaleAvailable ~= false,
            scale = tonumber(data.markerScale) or tonumber(self.currentMarkerScale) or MARKER_DEFAULT,
            override = settings.markerScaleOverride == true,
        },
        checks = {
            revision = tonumber(self.manualCheckRevision) or 0,
            lastKind = tostring(data.manualCheckKind or ""),
            lastAt = tonumber(data.manualCheckAt) or 0,
        },
        capabilities = caps,
    }
end

function U:GetManualCheckSnapshot(kind, limit)
    kind = tostring(kind or "buff")
    local source = type(self.manualChecks) == "table" and self.manualChecks[kind] or nil
    if type(source) ~= "table" then
        return { kind = kind, revision = 0, checked = 0, rows = {}, message = "尚未执行检查" }
    end
    return {
        kind = kind,
        revision = tonumber(source.revision) or 0,
        checkedAt = tonumber(source.checkedAt) or 0,
        checked = tonumber(source.checked) or 0,
        ok = source.ok == true,
        message = source.message,
        buffReadable = source.buffReadable,
        hiddenReadable = source.hiddenReadable,
        unreadableCount = tonumber(source.unreadableCount) or 0,
        roleCounts = CopyCounts(source.roleCounts),
        rows = CopyRows(source.rows, limit or 100),
    }
end

function U:GetSacrificeSnapshot(limit)
    local rows = {}
    local now = S.NowMs ~= nil and S.NowMs() or 0
    for unitId, candidate in pairs(type(self.candidates) == "table" and self.candidates or {}) do
        local active = type(self.activeSac) == "table" and self.activeSac[unitId] or nil
        local remaining = nil
        if type(active) == "table" then
            local elapsed = math.max(0, now - (tonumber(active.observedAt) or now))
            remaining = math.max(0, (tonumber(active.remainingMs) or SAC_DURATION_MS) - elapsed)
        end
        rows[#rows + 1] = {
            unitId = tostring(unitId),
            name = tostring((active and active.name) or (candidate and candidate.name) or unitId),
            active = active ~= nil,
            buffId = active and tonumber(active.buffId) or nil,
            remainingMs = remaining,
            state = active ~= nil and "施放中" or "候选",
        }
    end
    ...
    local maximum = math.max(1, math.floor(tonumber(limit) or 100))
    while #rows > maximum do table.remove(rows) end
    return {
        enabled = S.State.settings.sacMarkerEnabled == true,
        rows = rows,
        count = #rows,
        activeCount = tonumber((S.State.data.teamUtility or {}).sacActive) or 0,
    }
end
