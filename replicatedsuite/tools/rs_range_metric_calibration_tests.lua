...
Test('aspect-safe camera normalizes non-unit camera direction before range calibration and rigid projection',function()
    nowMs=3500; screenW=1920; screenH=1080
    UIParent.GetViewCameraDir=function() return {x=0,y=2,z=0} end
    targetWorldX=60;targetDistanceMeters=30
    local cx,cy=screenW/2,screenH/2
    local normalizedCameraDelta=(60/100)*(screenH/2)
    nativeTargetX=cx+normalizedCameraDelta*1.2
    X2Unit.GetUnitScreenPosition=function(self,token)
        if token=='player' then return cx,cy,1 end
        if token=='target' then return nativeTargetX,cy,1 end
        return nil,nil,nil
    end
    local sample=P:MeasureRangeMetricCalibration({anchorUnit='player',targetUnit='target',anchorWorld={x=0,y=0,z=0},worldZOffset=0.25,aspectSafeCamera=true})
    assert(sample.aspectSafeCamera==true,'aspect-safe camera flag missing')
    Near(sample.rawCameraDirLength,2,0.0001,'raw camera direction length')
    assert(sample.projectionScaleStatus=='accepted','aspect-safe scale rejected: '..tostring(sample.projectionScaleReason))
    Near(sample.projectionScale,1.2,0.0001,'aspect-safe projection scale')

    local projected,source,facts=P:ProjectWorldBatch({
        {x=10,y=0,z=0.25}, -- camera-right offset
        {x=0,y=0,z=10.25}, -- camera-up offset
    },{easyPullCompat=true,rigidBatch=true,aspectSafeCamera=true,anchorUnit='player',anchorWorld={x=0,y=0,z=0.25}})
    assert(source=='easypull_camera','expected camera fallback, got '..tostring(source))
    assert(facts.aspectSafeCamera==true,'batch did not report aspect-safe camera')
    Near(facts.rawCameraDirLength,2,0.0001,'batch raw camera direction length')
    local dx=math.abs((projected[1] and projected[1].x or 0)-cx)
    local dy=math.abs((projected[2] and projected[2].y or 0)-cy)
    Near(dx,dy,0.01,'orthonormal camera right/up pixel scale')

    UIParent.GetViewCameraDir=function() return {x=0,y=1,z=0} end
end)
...
Test('range feature consumes metric calibration without changing persisted meter radius',function()
    ...
    assert(text:find('aspectSafeCamera = true',1,true) or text:find('aspectSafeCamera=true',1,true),'range feature does not request aspect-safe camera projection')
