...
- **语义父导航（`.18.192`）**：隐藏子 route 可以携带 `navigationParentRoute` 作为主导航选中态提示，但 `PageHost`、Router current、Feature Consumer 与生命周期必须继续使用真实子 route。父导航只属于 Presentation Selection Authority，禁止变成业务路由重写或 Feature 强耦合。
- **Compact Plugin Surface（`.18.192`）**：状态/帮助型页面卡片默认 `auto` 而不是 `fill`；Floating HUD 优先压缩控制行数并保留稳定 Component ID。纯视觉尺寸升级不得为了“一次性迁移”随意改 Store canonical/schema；若历史精确默认尺寸需要兼容，可在读取后的 Presentation 副本做有界映射，并保留其他用户自定义 geometry。
...
- `.18.117` 后 live page 的 Keyboard eligibility 改为**显式点击临时武装**：EditBox 创建时 `EnableKeyboard(false)`。`.18.156` 进一步把激活拆成“Keyboard admission + Focus admission”：若点击使 `ArmInputWidget` 发生 `false→true`，即使 RU 已先把 Global Focus ID 指向 EditBox，也必须在 promotion 后执行一次 `SetFocus`；只有已经 armed+focused 的重复点击可跳过。LostFocus/隐藏/禁用/失去 Pick/Runtime quiesce 统一 `DisarmInputWidget`。Component/Owner Release 和 old-generation hot reload 仍额外永久 `EnableFocus(false)` 并注销，retirement 幂等。该设计避免 RU 在“已 EnableKeyboard 但未真正编辑”时吞掉 WASD/技能键。
