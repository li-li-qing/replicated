...
1. **只有两个按钮**：打开背包 + 银行（或箱子），背包上方快捷条必须只有 `取` 与 `放`，且右侧状态文字完整可读（不再是 74px 的截断小条）。整理背包页"日常整理"同样只剩 `取同类 / 放同类`。
2. **死锁回归（最关键一条）**：先制造"两边没有共同同类"的情形（例如银行里全是背包没有的东西），点一次 `放` → 应显示 `没有同类物品` 并说明可用「高级整理」；**紧接着**再点 `放`/`取` 必须照常工作，不得出现"点了没反应"或"快捷取放已经在运行"。旧版在这一步会把取放、页面直接移动、高级整理全部永久锁死，只有 `停` 或 Reload 能救。
3. **停止与切换**：一次能移动多件时点 `放`，右侧临时显示进度 `正在放入 n/N`（v8 起不再带"银行/箱子"前缀）；运行中再点 `放` 必须立刻停止（不再移动新物品，已移动的不回滚）；运行中点 `取` 必须切换方向且只剩一个移动任务。
4. **自愈**：若某轮取放中途被调度器打断（例如连续异常熔断），最迟下一次 350ms 观察后状态应变为 `已自动停止` 并带原因，随后点击可用——不得再出现"永远运行中"。
5. **拒绝必须有原因**：容器读取失败/窗口未开/黑名单命中这类拒绝，快捷条显示短句，页面状态行显示完整长句（含槽位/原因）。没有"静默无反应"这一档。
6. **不该变的保持不变**：250ms 串行限速、每步黑名单复验、任一槽读取失败即整次拒绝（保守 fail-closed）、仅打开窗口不扫描物品、关闭仓储窗口后隐藏、显式关闭 `tools_bag` 后不再出现悬浮条。
7. **提示不被裁切（本轮截图暴露的缺陷）**：分别悬停 `取` 与 `放`，提示框必须完整显示那两句短句——不得出现上边缘切掉半行字、或左半截文字缺失；再悬停任意表格行（跑商 / 活动列表）里被省略号截断的文字，提示同样要完整。若仍被裁，请把该提示**连同完整文字**一起截图：我需要区分"框比文字矮"（Tooltip v5 的估计问题）与"被某个窗口盖住"（层级问题），两者修法完全不同。
8. **提示必须停在最上层（本轮第二次回报）**：把鼠标放到 `取` 或 `放` 上停住不动 3~5 秒。提示必须一直完整可见、一直压在悬浮条与原生背包窗之上，不得出现"先在上面、不到 1 秒又沉下去"。旧版根因：悬浮条每 350ms 无条件 `Raise()` 自己，把提示抢了下去。同时确认提示期间悬浮条本身仍可见（只是不再抢层级），移开鼠标后提示立刻消失。
9. **悬浮条平时只有两个按钮（本轮第四次回报）**：打开银行或箱子后，背包上方应当只有 `取` `放` 两个按钮和一条窄背景，**没有**常显的 `银行 · 可快捷取放`。点一次无可移动同类的 `放`，右侧应临时出现 `没有同类物品`，约 6 秒后自动消失、条宽缩回两按钮；运行中显示 `正在放入 3/12` 这类进度且运行期间不消失。
10. **悬浮条第一次没出现的情况**：如果再次发生"打开箱子但背包上没有按钮"，请先别关箱子，打开 整理背包 页面，把状态行整行抄回（形如 `箱子=ready/true/main-script` / `银行=unknown/false/none`）——那一行直接区分"窗口事实没读到"和"读到了但 Presenter 没建出来"。
11. **高级整理不再问你目标**：开着箱子时点「开始整理」，物品必须进**箱子**；换成开着银行时点，必须进**银行**；两个都没开时点，必须看到"请先打开银行或箱子"而不是静默无反应。页面上原来那个开关位置现在是一句只读回显（如 `目标：箱子（当前打开）`），不应该能点击。
12. **满仓继续**（`.18.127` 语义复验）：仓库无空槽但 B 有未满同类堆时，A 局部跳过、B 必须继续堆叠。
13. 若仍能复现"点了没反应"：把快捷条整行文字 + 页面状态行（含 `悬浮按钮 N 个`）+ 诊断页 `tools_bag` 行抄回，作为下一轮唯一证据；**不要因为"看起来是 UI 问题"就去改移动语义**。
...
6. **Bag 快捷按钮**：打开银行/箱子，`取 / 放` 两个按钮（`.18.183` 起无 `停`，停止＝再点同一个按钮）应继续跟随当前背包 Native 窗口，切分辨率后重新开窗口仍正确；它不保存物理像素，因此不应受旧分辨率位置污染。
...
6. **整理背包 UX + 满仓继续**：鼠标分别悬停 `取/放`，应在指针附近显示准确说明（说明里必须写清"再点一次＝停止、点另一个＝切换方向"）。仓库无空槽但 B 物品存在未满同类堆时，A 无法放入只能局部跳过，B 必须继续堆叠；Native 读失败/identity 不可证明仍全局停止。
