...
- `RU-BAG-QUICK-183`：**用户报"第三个按钮没用 + 有时点了没反应"，实机现象背后是一个真死锁**。`.18.183` 根因：`BeginBagQuick` 在 `plannedMoves == 0` 早退前就把空 queue 表赋给 `_quickQueue`，而 `BagQuickRunning` 视"`_quickQueue ~= nil`"为运行中 → 一次"背包/仓库没有共同同类"的点击会永久占住互斥锁，此后 `取/放`、页面直接移动、类别批量整理全部被"已经在运行"拒绝，而被拒绝那次仍 `return true`，界面毫无反馈。修复三层：① 空计划不再持锁（`QuickQueueActive` 只认非空队列/在途写）；② `ReclaimStaleBagQuickRun` 复用既有 350ms 窗口观察任务做看门狗（任务未注册 / 8s 无推进即释放并说明原因，无新增 Tick/任务）；③ 快捷条与页面按钮收敛为 `取 / 放`，同向再点＝停止、异向＝切换，状态拆成短 `status` + 长 `error`。证据：Lua 行为模拟器修复前 14 红 / 修复后 39 绿，7 个反向破坏用例全部 CAUGHT。**RU 必须复验的是"没有同类→点放→随后取放仍可用"这条路径**，本地 Harness 不能关闭它。

- 同轮用户第三次回报：高级整理的「目标：银行/箱子」开关被判为歧义（两者不可能同时打开）。目标改为**当前真正打开的那个仓储窗口**，与快捷取放共用 `CurrentStorageContext` 事实源，页面只回显解析结果、不再提供选择；`DepositCategoryBank/Coffer` 与 `SetBatchTarget` 保留给显式调用方与旧存档，`State.batchTarget` 降级为 legacy 字段（投影新增 `batchTargetMode/batchTargetResolved`）。 同轮第四次回报把悬浮条右侧那句常显 `银行 · 可快捷取放` 判为冗余（"只要显示取跟放就行了，游戏插件简单方便最重要"）：presenter v8 改为 **quiet by default** —— 标签默认空且隐藏、条宽收成两按钮的 102px；只有"运行中进度（永不过期，替代停的可发现性）"与"一次点击的短结论（约 6 秒后自动消失并缩回）"会临时撑开它。每次状态写入都带 `statusAt`（`QuickStatusTimestampContractVersion=1`），过期判定复用既有 350ms 心跳，不新增 Tick/任务。
...
| Bag 整理按钮点击没反应 | `.18.82` 修命令链；**`.18.183` 找到并修掉真死锁：空计划永久占用 quick mutex，后续点击全被静默拒绝** | 已按 `OnClick → Feature Command → 队列生命周期 → Action Result` 全链复验；保留 250ms 限速/互斥/黑名单/fail-closed，只把拒绝变成可见 + 自愈 | Bag 主能力 IMPLEMENTED；quick overlay 两按钮化 + 自愈待 RU 关闭 |
...
| `RU-BAG-02` | **P0 业务回归** | **快捷条出现 3 个按钮，第三个（停）"没有用"；而且"有时点了存/放没有效果，像是卡到什么了"。** 用户把同一件事的两侧当成了两个问题。 | `.18.183` 根因见 §1 `RU-BAG-QUICK-183`：空计划占住 quick mutex → 之后所有取放/直接移动/类别批量都被"已经在运行"拒绝且没有任何反馈；`停` 实际是唯一解锁手段，但它在常态下没有任何作用。 | 空计划不持锁（`QuickQueueActive`）+ 350ms 窗口观察任务兼任看门狗（任务未注册 / 8s 无推进即释放并给出原因）+ 快捷条与页面只留 `取/放`（同向再点＝停止、异向＝切换）+ `status` 短/`error` 长双通道。行为模拟器 39/39 + 7 破坏用例已锁。 | 背包与仓库无同类时点一次 `放`，随后 `取`/`放`/高级整理都仍可执行；运行中再点同向按钮立即停止且不再移动新物品；切换方向不留第二个任务；任何被拒点击都要在快捷条或页面状态行看到原因。 |
