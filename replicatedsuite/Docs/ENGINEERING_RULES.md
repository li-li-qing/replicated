...
14. **Component API / Release Coherence 是封包硬门禁**：Composite/Workspace 不得假设 Primitive 私有 convenience method；公共行为优先走 RSUI Base Contract，类型特有方法必须在构建边界通过 `RequireComponentMethods()` 明示验证。任何 CURRENT/Gate 已声明的运行时 Contract 必须与实现、诊断入口、专项 Harness 同包存在；禁止只升级文档/Gate 或只发增量文件导致“版本看似前进、实现实际缺失”。所有 Presentation 交付必须同时通过 `rs_rsui_component_api_audit.py` 与 `rs_presentation_feature_api_audit.py`：前者约束 Component Public API，后者约束 Presentation→Feature/Commands Public API；未知 Component/Feature 方法除非有显式 capability guard 否则禁止封包。`rs_rsui_workspace_smoke_harness.py` 必须真实构建 MasterDetail / InspectorWorkbench / ResponsiveInspector / LayoutEditor / SettingsWorkbench / CommandCenter 六类公共模板；Foundation Audit 必须验证 RSUI 顶层 fail-closed 依赖的 TOC provider-before-consumer 顺序。Persistence Fresh Reload Contract 相关改动必须跑 `rs_persistence_acceptance_snapshot_harness.py`；任何专项 Harness 的默认工程根必须由脚本自身位置解析，禁止依赖调用者当前工作目录。
