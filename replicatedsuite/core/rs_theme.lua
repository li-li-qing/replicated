...
local function ActiveFontScale()
    if S.AppState ~= nil and type(S.AppState.settings) == "table" then return tonumber(S.AppState.settings.fontScale) or 1 end
    return 1
end
local C = S.Constants
...
local function Token(path, fallback)
    ...
    if tokens ~= nil and type(tokens.Get) == "function" then return tokens:Get(path, fallback) end
    return fallback
end

local function TokenNumber(path, fallback)
    local tokens = S.UITokens
    if tokens ~= nil and type(tokens.Number) == "function" then return tokens:Number(path, fallback) end
...
local function ResolveAlign(value)
    if type(value) == "number" then return value end
    local normalized = tostring(value or ""):lower():gsub("[^%w]", "")
    if normalized == "center" or normalized == "aligncenter" then return ALIGN_CENTER end
    if normalized == "right" or normalized == "alignright" then return ALIGN_RIGHT end
    if normalized == "topleft" or normalized == "aligntopleft" then return ALIGN_TOP_LEFT or ALIGN_LEFT end
    if normalized == "left" or normalized == "alignleft" then return ALIGN_LEFT end
    return ALIGN_LEFT
end


function T:ResolveFontSize(baseSize, localScale)
    local base = tonumber(baseSize) or TokenNumber("font.bodyLarge", 12)
    local layoutScale = (S.Layout and S.Layout.GetContext) and S.Layout:GetContext().addonScale or 1
    local fontScale = ActiveFontScale()
    local localMultiplier = math.max(0.50, math.min(2.00, tonumber(localScale) or 1.0))
    -- Preserve the historical global 10..24 native range when no local HUD
    -- scaling is requested. Floating surfaces may opt into a bounded local
    -- multiplier; give that path a slightly wider safe range without changing
    -- ordinary page typography.
    local minimum = localMultiplier == 1.0 and 10 or 8
    local maximum = localMultiplier == 1.0 and 24 or 32
    return math.max(minimum, math.min(maximum, math.floor(base * layoutScale * fontScale * localMultiplier + 0.5)))
end
...
local function CopyColor(color, fallback)
    ...
    return {
        tonumber(color and color[1]) or tonumber(fallback[1]) or 1,
        tonumber(color and color[2]) or tonumber(fallback[2]) or 1,
        tonumber(color and color[3]) or tonumber(fallback[3]) or 1,
        tonumber(color and color[4]) or tonumber(fallback[4]) or 1,
    }
end

local function ApplyTextColor(widget, baseColor, opacity)
    if widget == nil or widget.style == nil then return false end
    local base = CopyColor(baseColor, { 1, 1, 1, 1 })
...
function T:ToneColor(tone)
    ...
    if tone == "green" or tone == "success" then return colors.green end
    if tone == "yellow" or tone == "warning" then return colors.yellow end
    if tone == "orange" or tone == "caution" then return colors.orange end
    if tone == "red" or tone == "danger" then return colors.red end
    if tone == "blue" or tone == "info" then return colors.blue end
    if tone == "purple" then return colors.purple end
    if tone == "accent" then return colors.accent end
...
function T:StyleLabel(label, size, tone, align, shadow)
    ...
    label.rsBaseFontSize = tonumber(size) or TokenNumber("font.bodyLarge", 12)
    if label.style.SetFontSize ~= nil then
        local applied = self:ResolveFontSize(label.rsBaseFontSize, label.rsLocalFontScale)
        label.style:SetFontSize(applied)
        label.rsAppliedFontSize = applied
    end
...
function T:StyleButton(button, width, height, fontSize, active, useGradient)
    ...
    height = math.max(1, tonumber(height) or TokenNumber("size.buttonH", 26))
    ...
    local normal = active and (buttonTokens.active or { 0.17, 0.14, 0.09, 0.98 }) or (buttonTokens.normal or { 0.075, 0.095, 0.120, 0.97 })
    local highlight = active and (buttonTokens.activeHover or { 0.26, 0.21, 0.12, 0.99 }) or (buttonTokens.hover or { 0.14, 0.19, 0.24, 0.99 })
    local pushed = buttonTokens.pushed or { 0.045, 0.060, 0.078, 0.99 }
    local disabled = buttonTokens.disabled or { 0.050, 0.052, 0.058, 0.70 }
    ...
    if button.style ~= nil then
        if button.style.SetFontSize ~= nil then
            local applied = self:ResolveFontSize(button.rsBaseFontSize, button.rsLocalFontScale)
...
function T:SetOpacity(widget, opacity)
    if widget == nil or widget.SetAlpha == nil then return false end
    local value = math.max(0.0, math.min(1.0, tonumber(opacity) or 1.0))
    if widget.rsAppliedAlpha == value then return false end
    widget:SetAlpha(value)
    widget.rsAppliedAlpha = value
    return true
end
...
function T:SetTextOpacity(widget, opacity)
    ...
    return ApplyTextColor(widget, base, opacity)
end


function T:SetEllipsis(widget, enabled)
    if widget == nil or widget.style == nil or type(widget.style.SetEllipsis) ~= "function" then return false end
    local value = enabled == true
    if widget.rsEllipsis == value then return false end
    pcall(function() widget.style:SetEllipsis(value) end)
    widget.rsEllipsis = value
    return true
end

function T:SetLabelTone(label, tone)
    if label == nil or label.style == nil then return false end
    ...
    for _, widget in pairs(controls) do
        if widget ~= nil and widget.style ~= nil and widget.rsBaseFontSize ~= nil and widget.style.SetFontSize ~= nil then
            ...
            if widget.rsAppliedFontSize ~= applied then
                ...
                if ok then
                    widget.rsAppliedFontSize = applied
                    changed = changed + 1
                    if S.UI ~= nil and type(S.UI.PrimeNativeState) == "function" then S.UI:PrimeNativeState(widget, { fontSize = applied }) end
                end
            end
        end
    end
    if changed > 0 and S.RSUI ~= nil and type(S.RSUI.InvalidateTypography) == "function" then
        S.RSUI:InvalidateTypography("theme_typography_changed")
    end
    return changed
end
