...
local function RebuildCurrentTeamAliases()
    -- Group every roster entity that owns each normalized name. A name owned by
    -- more than one entity is a conflict, but we no longer DROP those events: a
    -- deterministic winner keeps the damage attributed (and therefore visible in
    -- the team ranking) so it is never lost. The manual correction feature lets
    -- the user reassign mis-attributed rows afterward, so a wrong attribution is
    -- recoverable while a dropped row is not.
    local candidates = {}
    for rosterKey, record in pairs(E.roster or {}) do
        ...
        if type(entity) == "table" then
            for _, normalized in ipairs(type(record) == "table" and record.aliases or {}) do
                if normalized ~= "" then
                    local bucket = candidates[normalized]
                    if bucket == nil then
                        bucket = { count = 0 }
                        candidates[normalized] = bucket
                    end
                    if bucket[entity.key] == nil then
                        bucket[entity.key] = entity
    ...
    for normalized, bucket in pairs(candidates) do
        if bucket.count <= 1 then
            for key, entity in pairs(bucket) do
                if key ~= "count" then owners[normalized] = key end
            end
        else
            -- Conflict: choose a deterministic winner so data is retained rather
            -- than rejected. Prefer a stable id:* entity; tie-break by most
            -- recently seen, then by key for full stability across runs.
            local winnerKey = nil
            for key, entity in pairs(bucket) do
                if key ~= "count" then
                    if winnerKey == nil then
                        ...
                        local ek = tostring(key)
                        local wk = tostring(winnerKey)
                        local eId = string.sub(ek, 1, 3) == "id:"
                        local wId = string.sub(wk, 1, 3) == "id:"
                        if eId and not wId then
                            winnerKey = key
                        elseif eId == wId then
                            local eSeen = tonumber(entity and entity.lastSeenAt) or 0
                            local wEnt = bucket[winnerKey]
                            local wSeen = tonumber(wEnt and wEnt.lastSeenAt) or 0
                            if eSeen > wSeen or (eSeen == wSeen and ek < wk) then
                                winnerKey = key
                            end
                        end
                    end
            ...
            if winnerKey ~= nil then owners[normalized] = winnerKey end
...
function R:FinalizeRosterScan(job)
    ...
    -- Fold same-token roster duplicates (joint-raid page switch / unstable unit
    -- id produced id:<a> + id:<b> or teamname:<name> + id:<b> for one slot) into a
    -- single entity before the alias map is rebuilt, so the conflict lock that was
    -- blocking those members' damage events is cleared.
    local collapsed = E:CollapseSameTokenRosterDuplicates(now)
