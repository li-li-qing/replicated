...
-- builder publishes an alias for exactly one current roster entity; on a name
-- collision it publishes a *deterministic winner* rather than omitting the name.
-- Dropping the alias would discard every damage event from that name, and lost
-- data cannot be recovered — a wrong attribution can, via the manual correction
-- feature. The winner is chosen in RebuildCurrentTeamAliases (stable id: first,
...
-- Collapse roster records that share one token but resolved to different entity
-- keys. Joint-raid page switches and unstable unit IDs can register the SAME
-- member as both teamname:<name> and id:<id>, or as two distinct id:<id> values
-- for one slot. They collide on the normalized name, trip nameConflicts, and then
-- every damage event from that name is rejected as CONTEXT_ONLY_TEAM_SCOPE —
-- the "2 people's data can't come in" symptom. A shared roster token is strong
-- proof of a single player (token = schemeIndex*1000+tokenIndex = one slot), so
-- folding the duplicates into one entity is safe, unlike a broad name merge.
--
-- Genuine same-name ambiguity (two different players, hence different tokens) is
-- left untouched: those records never share a token, so they are not collapsed
-- and remain fail-closed.
function E:CollapseSameTokenRosterDuplicates(seenAt)
    local byToken = {}
    for key, record in pairs(self.roster or {}) do
        if type(record) == "table" and record.token ~= nil then
            local t = tostring(record.token)
            byToken[t] = byToken[t] or {}
            byToken[t][#byToken[t] + 1] = key
        end
    end

    local changed = false
    local mergedWinners = {}
    for token, keys in pairs(byToken) do
        if #keys > 1 then
            -- Prefer a stable id:* key as the survivor; otherwise keep the first.
            local winnerKey = nil
            for _, key in ipairs(keys) do
                if winnerKey == nil then
                    winnerKey = key
                elseif string.sub(tostring(key), 1, 3) == "id:" and string.sub(tostring(winnerKey), 1, 3) ~= "id:" then
                    winnerKey = key
                end
            end
            local winnerEntity = self:GetByKey(winnerKey)
            if type(winnerEntity) ~= "table" then
                -- winner key no longer resolves; leave the group for manual repair
            else
                for _, key in ipairs(keys) do
                    if key == winnerKey or self.roster[key] == nil then
                        -- keep winner / skip already-removed record
                    else
                        local loserEntity = self:GetByKey(key)
                        if type(loserEntity) == "table" and loserEntity ~= winnerEntity then
                            -- Route every future lookup of the loser key to winner.
                            self.aliases[key] = winnerKey
                            self.byKey[key] = nil
                            if self.teamMissingSince ~= nil then
                                self.teamMissingSince[winnerKey] = self.teamMissingSince[winnerKey] or self.teamMissingSince[key]
                                self.teamMissingSince[key] = nil
                            end
                            if D.Stats ~= nil and D.Stats.MergeActorKey ~= nil then
                                local rec = self.roster[key]
                                D.Stats:MergeActorKey(key, winnerKey, rec and rec.name or nil)
                            end
                        end
                        -- Union the loser's aliases into the winner record so the
                        -- single surviving entity owns every spelling.
                        local wrec = self.roster[winnerKey]
                        local lrec = self.roster[key]
                        if type(wrec) == "table" and type(lrec) == "table" and type(lrec.aliases) == "table" then
                            wrec.aliases = wrec.aliases or {}
                            local seen = {}
                            for _, a in ipairs(wrec.aliases) do seen[a] = true end
                            for _, a in ipairs(lrec.aliases) do
                                if not seen[a] then
                                    seen[a] = true
                                    wrec.aliases[#wrec.aliases + 1] = a
                                end
                            end
                        end
                        self.roster[key] = nil
                ...
                mergedWinners[winnerKey] = true
            end
        end
    end

    if changed then
        -- Count how many surviving roster records still carry each normalized
        -- name. The conflict lock is cleared only for names that are now unique:
        -- a shared token proves one player, so a same-name that survives only
        -- once is no longer ambiguous and must resolve again.
        local nameCount = {}
        for _, record in pairs(self.roster or {}) do
            if type(record) == "table" and type(record.aliases) == "table" then
                local seen = {}
                for _, a in ipairs(record.aliases) do
                    if not seen[a] then
                        seen[a] = true
                        nameCount[a] = (nameCount[a] or 0) + 1
                    end
                end
            end
        end
        for winnerKey in pairs(mergedWinners) do
            local wrec = self.roster[winnerKey]
            local winnerEntity = self:GetByKey(winnerKey)
            if type(wrec) == "table" and type(wrec.aliases) == "table" then
                for _, normalized in ipairs(wrec.aliases) do
                    if type(normalized) == "string" and normalized ~= ""
                        and (nameCount[normalized] or 0) <= 1
                        and self.nameConflicts[normalized] == true then
                        self.nameConflicts[normalized] = nil
                        if type(winnerEntity) == "table" and type(winnerEntity.flags) == "table" then
                            winnerEntity.flags.nameConflict = nil
                        end
                        self.byName[normalized] = winnerKey
                        self.aliases["name:" .. normalized] = winnerKey
                    end
                end
            end
        end
        D.Diagnostics.counters.rosterSameTokenCollapsed =
            (tonumber(D.Diagnostics.counters.rosterSameTokenCollapsed) or 0) + 1
    end
    return changed
end
