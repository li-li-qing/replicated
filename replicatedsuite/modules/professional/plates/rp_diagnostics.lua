...
function D:BuildReport()
    ...
    if type(runtime)=="table" then
        Add("SuccessfulUpdates=" .. tostring(runtime.successfulUpdateSerial or 0)
            ...
            .. "  WatchdogAttempts=" .. tostring(runtime.watchdogRecoveryAttempts or 0)
            .. "  WatchdogSuccess=" .. tostring(runtime.watchdogRecoverySuccesses or 0)
            .. "  WatchdogBudgetDeferred=" .. tostring(runtime.watchdogBudgetDeferrals or 0)
            .. "  VisibilityRepairs=" .. tostring(runtime.visibilityRepairs or 0))
        local budget = runtime.budget or {}
        Add("FrameBudget requests=" .. tostring(budget.requests or 0)
            .. " granted=" .. tostring(budget.granted or 0)
            .. " deferred=" .. tostring(budget.deferred or 0)
            .. " starvation=" .. tostring(budget.starvation or 0))
        local rows = {}
        for label, lane in pairs(budget.byLane or {}) do
            if (tonumber(lane.deferred) or 0) > 0 then
                rows[#rows + 1] = { label=label, deferred=tonumber(lane.deferred) or 0, max=tonumber(lane.maxConsecutiveDefers) or 0 }
            end
        end
        table.sort(rows, function(a,b) return a.deferred ~= b.deferred and a.deferred > b.deferred or tostring(a.label) < tostring(b.label) end)
        for i = 1, math.min(6, #rows) do
            Add("  deferred " .. tostring(rows[i].label) .. "=" .. tostring(rows[i].deferred) .. " maxConsecutive=" .. tostring(rows[i].max))
        end
        ...
        local ui = P.UI ~= nil and type(P.UI.GetPerformanceSnapshot) == "function" and P.UI:GetPerformanceSnapshot() or nil
        if type(ui) == "table" then
            local e, l, c = ui.effects or {}, ui.lines or {}, ui.circle or {}
            Add("UI Diff effects updates=" .. tostring(e.updates or 0) .. " visible=" .. tostring(e.visible or 0) .. " peak=" .. tostring(e.peakVisible or 0)
                .. " hidden=" .. tostring(e.hidden or 0) .. " textureChanges=" .. tostring(e.textureChanges or 0))
            Add("UI Diff lines frames=" .. tostring(l.frames or 0) .. " active=" .. tostring(l.active or 0) .. " peak=" .. tostring(l.peakActive or 0)
                .. " staleHides=" .. tostring(l.staleHides or 0))
            Add("UI Diff circle frames=" .. tostring(c.frames or 0) .. " active=" .. tostring(c.active or 0) .. " peak=" .. tostring(c.peakActive or 0)
                .. " staleHides=" .. tostring(c.staleHides or 0))
            for _, row in ipairs(ui.frameworkOwners or {}) do
                Add("  Framework " .. tostring(row.owner or "?") .. " attempts=" .. tostring(row.attempts or 0) .. " writes=" .. tostring(row.writes or 0)
                    .. " skips=" .. tostring(row.skips or 0) .. " native=" .. tostring(row.nativeCalls or 0))
            end
        end
    end
    Add("")
