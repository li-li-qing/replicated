...
local U = P.UI
local SuiteUI = ReplicatedSuite ~= nil and ReplicatedSuite.UI or nil
local LocalDiffState = setmetatable({}, { __mode = "k" })
local function LocalState(widget)
    if widget == nil then return nil end
    local row = LocalDiffState[widget]
    if row == nil then row = {}; LocalDiffState[widget] = row end
    return row
end
local UI_OWNER = {
    effects = "plates:effects",
    lines = "plates:lines",
    circle = "plates:circle",
}

U.performance = {
    effects = { updates=0, hidden=0, textureChanges=0, visible=0, peakVisible=0 },
    lines = { frames=0, active=0, peakActive=0, staleHides=0 },
    circle = { frames=0, active=0, peakActive=0, staleHides=0 },
}

local function DiffText(widget, value, owner)
    if SuiteUI ~= nil and type(SuiteUI.SetText) == "function" then return SuiteUI:SetText(widget, value, owner) end
    if widget == nil or type(widget.SetText) ~= "function" then return false end
    local text = tostring(value or "")
    local row = LocalState(widget)
    if row.text == text then return false end
    widget:SetText(text); row.text = text; return true
end

local function DiffVisible(widget, visible, owner)
    if SuiteUI ~= nil and type(SuiteUI.SetVisible) == "function" then return SuiteUI:SetVisible(widget, visible == true, owner) end
    if widget == nil then return false end
    local value = visible == true
    local row = LocalState(widget)
    if row.visible == value then return false end
    if type(widget.Show) == "function" then widget:Show(value)
    elseif type(widget.SetVisible) == "function" then widget:SetVisible(value)
    else return false end
    row.visible = value
    return true
end

local function DiffExtent(widget, width, height, owner)
    if SuiteUI ~= nil and type(SuiteUI.SetExtent) == "function" then return SuiteUI:SetExtent(widget, width, height, owner) end
    if widget == nil or type(widget.SetExtent) ~= "function" then return false end
    local w, h = tonumber(width) or 1, tonumber(height) or 1
    local row = LocalState(widget)
    if row.width == w and row.height == h then return false end
    widget:SetExtent(w, h); row.width, row.height = w, h; return true
end

local function DiffAnchor(widget, parent, x, y, owner)
    if SuiteUI ~= nil and type(SuiteUI.SetAnchor) == "function" then return SuiteUI:SetAnchor(widget, parent, x, y, owner) end
    if widget == nil or type(widget.AddAnchor) ~= "function" then return false end
    local ax, ay = tonumber(x) or 0, tonumber(y) or 0
    local row = LocalState(widget)
    if row.anchorParent == parent and row.anchorX == ax and row.anchorY == ay then return false end
    if type(widget.RemoveAllAnchors) == "function" then widget:RemoveAllAnchors() end
    widget:AddAnchor("TOPLEFT", parent, ax, ay)
    row.anchorParent, row.anchorX, row.anchorY = parent, ax, ay
    return true
end

local function DiffFontSize(widget, size, owner)
    if SuiteUI ~= nil and type(SuiteUI.SetFontSize) == "function" then return SuiteUI:SetFontSize(widget, size, owner) end
    if widget == nil or widget.style == nil or type(widget.style.SetFontSize) ~= "function" then return false end
    local value = tonumber(size)
    if value == nil then return false end
    local row = LocalState(widget)
    if row.fontSize == value then return false end
    widget.style:SetFontSize(value); row.fontSize = value; return true
end

local function DiffColor(drawable, r, g, b, a, owner)
    if SuiteUI ~= nil and type(SuiteUI.SetColor) == "function" then return SuiteUI:SetColor(drawable, r, g, b, a, owner) end
    if drawable == nil or type(drawable.SetColor) ~= "function" then return false end
    local row = LocalState(drawable)
    if row.colorR == r and row.colorG == g and row.colorB == b and row.colorA == a then return false end
...
local function SetIconPath(icon, path, owner)
    if icon == nil then return false end
    local managed = owner ~= nil
    local row = LocalState(icon)
    if type(path) ~= "string" or path == "" then
        if managed then DiffVisible(icon, false, owner) else icon:SetVisible(false) end
        row.texturePath = ""
        return false
    end
    -- Icon textures have no generic Suite setter on the RU client. Keep the
    -- native texture mutation behind the same local Diff contract as the
    -- geometry/color helpers so repeated effect/equipment refreshes do not
    -- clear and re-add an unchanged texture.
    if row.texturePath ~= path then
        icon:ClearAllTextures()
        icon:AddTexture(path)
        row.texturePath = path
    end
    if managed then DiffVisible(icon, true, owner) else icon:SetVisible(true) end
    return true
end
...
local function CreateEffectSlot(parent, id, effectType)
    ...
    local record = {
        ...
        tooltipEnabled = false, tooltipText = nil, effect = nil, pickable = false,
    }
...
local function HideEffectSlots(slots)
    for _, slot in ipairs(slots) do
        slot.key, slot.durationText, slot.stackText = nil, nil, nil
        if slot.visible ~= false then
            DiffVisible(slot.frame, false, UI_OWNER.effects)
            U.performance.effects.hidden = U.performance.effects.hidden + 1
        end
        slot.visible = false
        DiffVisible(slot.icon, false, UI_OWNER.effects)
        slot.effect = nil
        slot.tooltipEnabled = false
        slot.tooltipText = nil
        if slot.pickable ~= false then
...
local function ApplySlotColor(drawable, color, fallback, owner)
...
local function UpdateEffectSlot(slot, effect, layout)
    U.performance.effects.updates = U.performance.effects.updates + 1
    ...
    if slot.iconPath ~= effect.iconPath then
        ...
        U.performance.effects.textureChanges = U.performance.effects.textureChanges + 1
        SetIconPath(slot.icon, effect.iconPath, UI_OWNER.effects)
    else
        DiffVisible(slot.icon, true, UI_OWNER.effects)
    end
    ...
    DiffExtent(slot.icon, baseIconSize, baseIconSize, UI_OWNER.effects)
    ...
    if slot.durationText ~= durationText then slot.durationText = durationText; DiffText(slot.duration, durationText, UI_OWNER.effects) end
    local stack = tonumber(effect.stack) or 0
    local stackText = showStack and stack > 1 and tostring(stack) or ""
    if slot.stackText ~= stackText then slot.stackText = stackText; DiffText(slot.stack, stackText, UI_OWNER.effects) end
    ...
    ApplySlotColor(slot.edge, edgeColor, fallbackColor, UI_OWNER.effects)
    DiffVisible(slot.edge, showBorder, UI_OWNER.effects)
    if expiring then
        local c = type(edgeColor)=="table" and edgeColor or fallbackColor
        DiffColor(slot.duration.style,
            tonumber(c.r or c[1]) or 1,
            tonumber(c.g or c[2]) or 0.3,
            tonumber(c.b or c[3]) or 0.2,
            1, UI_OWNER.effects)
    else
        DiffColor(slot.duration.style, 1, 1, 1, 1, UI_OWNER.effects)
    end
    ...
    if slot.pickable ~= showTooltip then
        SetPick(slot.frame, showTooltip)
        slot.pickable = showTooltip
    end
    if slot.visible ~= true then DiffVisible(slot.frame, true, UI_OWNER.effects) end
    slot.visible = true
end
...
function U:UpdateEffects(scope, effectType, effects, internalPreview)
    ...
    for index, slot in ipairs(slots) do
        if index <= maximum then
            UpdateEffectSlot(slot, effects[index], layout)
        elseif slot.visible ~= false then
            slot.visible = false
            DiffVisible(slot.frame, false, UI_OWNER.effects)
            U.performance.effects.hidden = U.performance.effects.hidden + 1
        end
    ...
    local visible = 0
    for _, plate in pairs(self.plates or {}) do
        for _, count in pairs(plate.effectCounts or {}) do visible = visible + (tonumber(count) or 0) end
    end
    U.performance.effects.visible = visible
    U.performance.effects.peakVisible = math.max(U.performance.effects.peakVisible, visible)
end
...
------------------------------------------------------------------------
U.linesHost = nil
U.linesHostVisible = false
U.linesPools = {}   -- [pairKey] = { dots = {label,...}, activeCount = number }
...
local function PrimeDot(dot)
    if SuiteUI == nil or type(SuiteUI.PrimeNativeState) ~= "function" then return end
    SuiteUI:PrimeNativeState(dot, { text=".", width=1, height=1, fontSize=15, visible=false })
    if dot.style ~= nil then SuiteUI:PrimeNativeState(dot.style, { colorR=1, colorG=1, colorB=1, colorA=1 }) end
end
...
local function EnsureLinesHost()
    ...
    host:Show(false)
    if SuiteUI ~= nil and type(SuiteUI.PrimeNativeState) == "function" then
        SuiteUI:PrimeNativeState(host, { width=200, height=200, visible=false })
    end
    return true
end

local function HideLinePool(pool)
    if type(pool) ~= "table" then return 0 end
    local previous = math.max(0, math.floor(tonumber(pool.activeCount) or 0))
    for i = 1, previous do DiffVisible(pool.dots[i], false, UI_OWNER.lines) end
    pool.activeCount = 0
    return previous
end

-- Push-driven incremental render. Only active dots are touched, and when a line
-- shrinks only the stale tail [newCount+1..oldCount] is hidden. Unused pool
-- capacity is never rewritten on a normal frame.
function U:UpdateLinesView(lines, cfg)
    U.performance.lines.frames = U.performance.lines.frames + 1
    if lines == nil or #lines == 0 then
        if U.linesHost ~= nil then
            local hidden = 0
            for _, pool in pairs(U.linesPools) do hidden = hidden + HideLinePool(pool) end
            U.performance.lines.staleHides = U.performance.lines.staleHides + hidden
            U.performance.lines.active = 0
            if U.linesHostVisible == true then DiffVisible(U.linesHost, false, UI_OWNER.lines) end
            U.linesHostVisible = false
        end
        return
    end
    ...
    for _, line in ipairs(lines) do
        ...
        if U.linesPools[key] ~= nil and type(line.points) == "table" then incoming[key] = line.points end
    end

    local totalActive, staleHides = 0, 0
    for _, key in ipairs(LINES_PAIR_ORDER) do
        local pool = U.linesPools[key]
        local points = incoming[key]
        local previous = math.max(0, math.floor(tonumber(pool.activeCount) or 0))
        local count = type(points) == "table" and math.min(#points, #pool.dots) or 0
        local color = LINES_PAIR_COLORS[key] or { 1, 1, 1 }
        for i = 1, count do
            local dot = pool.dots[i]
            local pt = points[i]
            local x = math.floor((tonumber(pt and pt.x) or 0) * scale + 0.5)
            local y = math.floor((tonumber(pt and pt.y) or 0) * scale + 0.5)
            DiffFontSize(dot, fontScaled, UI_OWNER.lines)
            if dot.style ~= nil then DiffColor(dot.style, color[1], color[2], color[3], alpha, UI_OWNER.lines) end
            DiffAnchor(dot, U.linesHost, x, y, UI_OWNER.lines)
            DiffVisible(dot, true, UI_OWNER.lines)
        end
        if previous > count then
            for i = count + 1, previous do DiffVisible(pool.dots[i], false, UI_OWNER.lines) end
            staleHides = staleHides + (previous - count)
        end
        pool.activeCount = count
        totalActive = totalActive + count
    end

    U.performance.lines.active = totalActive
    U.performance.lines.peakActive = math.max(U.performance.lines.peakActive, totalActive)
    U.performance.lines.staleHides = U.performance.lines.staleHides + staleHides
    local shouldShow = totalActive > 0
    if U.linesHostVisible ~= shouldShow then
        DiffVisible(U.linesHost, shouldShow, UI_OWNER.lines)
        U.linesHostVisible = shouldShow
        if shouldShow and U.linesHost.Raise ~= nil then pcall(function() U.linesHost:Raise() end) end
    end
end
...
U.circleHost = nil
U.circleDots = {}
U.circleActiveCount = 0
U.circleHostVisible = false
...
local function HideCircleActive()
    local previous = math.max(0, math.floor(tonumber(U.circleActiveCount) or 0))
    for i = 1, previous do DiffVisible(U.circleDots[i], false, UI_OWNER.circle) end
    U.circleActiveCount = 0
    U.performance.circle.staleHides = U.performance.circle.staleHides + previous
    U.performance.circle.active = 0
end

-- Incremental circle renderer: moving visible dots still update their anchors,
-- but static font/color/show writes are skipped, and unused pool capacity is
-- untouched. Shrinking a circle only hides the stale active tail.
function U:UpdateCircleView(screenPoints, cfg)
    U.performance.circle.frames = U.performance.circle.frames + 1
    if screenPoints == nil or #screenPoints == 0 then
        if U.circleHost ~= nil then
            HideCircleActive()
            if U.circleHostVisible == true then DiffVisible(U.circleHost, false, UI_OWNER.circle) end
            U.circleHostVisible = false
        end
        return
    end
    ...
    for i = 1, count do
        ...
        local x = math.floor((tonumber(pt and pt.x) or 0) * scale + 0.5)
        local y = math.floor((tonumber(pt and pt.y) or 0) * scale + 0.5)
        DiffFontSize(dot, fontScaled, UI_OWNER.circle)
        if dot.style ~= nil then DiffColor(dot.style, 0.55, 0.86, 1.00, alpha, UI_OWNER.circle) end
        DiffAnchor(dot, U.circleHost, x, y, UI_OWNER.circle)
        DiffVisible(dot, true, UI_OWNER.circle)
    end
    if previous > count then
        for i = count + 1, previous do DiffVisible(U.circleDots[i], false, UI_OWNER.circle) end
        U.performance.circle.staleHides = U.performance.circle.staleHides + (previous - count)
    end
    U.circleActiveCount = count
    U.performance.circle.active = count
    U.performance.circle.peakActive = math.max(U.performance.circle.peakActive, count)
    if U.circleHostVisible ~= true then
        DiffVisible(U.circleHost, true, UI_OWNER.circle)
        U.circleHostVisible = true
        if U.circleHost.Raise ~= nil then pcall(function() U.circleHost:Raise() end) end
    end
end

function U:GetPerformanceSnapshot()
    ...
    local owners = {}
    if SuiteUI ~= nil and type(SuiteUI.GetFrameworkSnapshot) == "function" then
        local snap = SuiteUI:GetFrameworkSnapshot()
        for _, row in ipairs(snap and snap.byOwner or {}) do
            if tostring(row.owner or ""):find("^plates:") ~= nil then owners[#owners + 1] = Copy(row) end
        end
    end
    return {
        version = "1.0",
        effects = Copy(self.performance and self.performance.effects),
        lines = Copy(self.performance and self.performance.lines),
        circle = Copy(self.performance and self.performance.circle),
        frameworkOwners = owners,
    }
end
