...
local function SharedMagicCircleBuffIds()
    local source = PlatesData and PlatesData.MagicCircleBuffIds or {}
    local result = {}
...
P.Storage = {
    ...
    -- Transient presentation revisions.  These are not persisted and do not
    -- change Authority semantics; they let RSUI consumers invalidate cached
    -- projections without re-sorting large tracking buckets every refresh.
    revision = 0, trackingRevision = 0,
...
local function Defaults()
    return {
        ...
        magiccircle = { enabled = false, offsetX = 200, offsetY = -6, fontSize = 11, alpha = 95,
                        warnM = 25, maxM = 29.9, buffIds = SharedMagicCircleBuffIds() },
...
-- These concern snapshots are intentionally split even though the mature
-- double-bank/sharded protocol remains in this file. Diagnostics must be able
-- to ask about one concern without calling Get()/Load() or entering Save().
function S:GetPersistenceHealth()
    return {
        schemaVersion = tonumber(P.SchemaVersion) or 0,
        loaded = self.settings ~= nil,
        dirty = self.dirty == true,
        revision = tonumber(self.revision) or 0,
        writeFence = self.writeFenceReason,
        futureSchemaVersion = self.futureSchemaVersion,
    }
end

function S:GetTrackingHealth()
    local entries = 0
    if self.settings ~= nil and type(self.settings.tracking) == "table" then
        for _, scope in ipairs(TRACKING_SCOPES) do
            local bucket = self.settings.tracking[scope]
            if type(bucket) == "table" then
                for _, effectType in ipairs(TRACKING_EFFECTS) do
                    entries = entries + BucketCount(bucket[effectType])
                end
            end
        end
    end
    return {
        dirty = self.trackingDirty == true,
        sharded = self.trackingSharded == true,
        storageVersion = tonumber(TRACKING_STORAGE_VERSION) or 0,
        futureVersion = self.trackingFutureVersion,
        entries = entries,
        revision = tonumber(self.trackingRevision) or 0,
    }
end

function S:GetAuraLibraryHealth()
    local entries = 0
    if self.settings ~= nil then entries = AuraLibraryCount(self.settings.auraLibrary) end
    return {
        dirty = self.auraDirty == true,
        sharded = self.auraSharded == true,
        storageVersion = tonumber(AURA_STORAGE_VERSION) or 0,
        futureVersion = self.auraFutureVersion,
        entries = entries,
        revision = tonumber(self.revision) or 0,
    }
end

-- Read-only facade consumed by Suite Diagnostics. It reports in-memory state
-- only; it never loads a manifest or performs a persistence API call. Flat
-- fields stay for existing consumers; the nested concerns are the new stable
-- diagnostic contract for finer-grained producers.
function S:GetHealth()
    local persistence = self:GetPersistenceHealth()
    local tracking = self:GetTrackingHealth()
    local aura = self:GetAuraLibraryHealth()
    return {
        version = "1.1",
        schemaVersion = persistence.schemaVersion,
        loaded = persistence.loaded,
        dirty = persistence.dirty,
        trackingDirty = tracking.dirty,
        auraDirty = aura.dirty,
        trackingSharded = tracking.sharded,
        auraSharded = aura.sharded,
        trackingStorageVersion = tracking.storageVersion,
        auraStorageVersion = aura.storageVersion,
        auraEntries = aura.entries,
        futureSchemaVersion = persistence.futureSchemaVersion,
        trackingFutureVersion = tracking.futureVersion,
        auraFutureVersion = aura.futureVersion,
        writeFence = persistence.writeFence,
...
function S:MarkTrackingDirty()
    self.dirty = true; self.trackingDirty = true
    self.revision = (tonumber(self.revision) or 0) + 1
    self.trackingRevision = (tonumber(self.trackingRevision) or 0) + 1
end
function S:MarkAuraDirty()
    self.dirty = true; self.auraDirty = true
    self.revision = (tonumber(self.revision) or 0) + 1
    -- Normal Buff/Debuff tracking is projected from AuraLibrary, so an aura
    -- mutation is also a tracking-presentation mutation.
    self.trackingRevision = (tonumber(self.trackingRevision) or 0) + 1
end
