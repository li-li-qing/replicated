...
local function CountDiscoveryEntries(discovered)
    local count = 0
    if type(discovered) ~= "table" then return count end
    for _, scopes in pairs(discovered) do
        if type(scopes) == "table" then
            for _, bucket in pairs(scopes) do count = count + CountMapEntries(bucket) end
        end
    end
    return count
end

local function CountCaptureEntries(capture)
    local count = 0
    if type(capture) ~= "table" or type(capture.buckets) ~= "table" then return count end
    for _, scopes in pairs(capture.buckets) do
        if type(scopes) == "table" then
            for _, bucket in pairs(scopes) do count = count + CountMapEntries(bucket) end
        end
    end
...
local function CopyDiscoveryCursor(cursor)
    local result = {}
    for scope, lanes in pairs(type(cursor) == "table" and cursor or {}) do
        result[scope] = {}
        if type(lanes) == "table" then
            for effectType, value in pairs(lanes) do result[scope][effectType] = tonumber(value) or 1 end
        end
    end
    return result
end

-- Read-only Manager concern snapshots for Suite Diagnostics. Every method
-- reports only in-memory session state: it never scans the RU client, loads or
-- saves settings, mutates queues, or resolves effect metadata.
function M:GetCatalogHealth()
    return {
        mode = tostring(self.catalogMode or "live"),
        categoryFilter = tostring(self.categoryFilter or "ALL"),
        entries = type(self.catalog) == "table" and #self.catalog or 0,
        trackedRows = type(self.trackedRows) == "table" and #self.trackedRows or 0,
        liveRows = type(self.liveRows) == "table" and #self.liveRows or 0,
        frozen = self.frozen == true,
        frozenScope = self.frozenScope,
        frozenEffect = self.frozenEffect,
    }
end

function M:GetDiscoveryHealth()
    return {
        serial = tonumber(self.discoverySerial) or 0,
        entries = CountDiscoveryEntries(self.discovered),
        cursor = CopyDiscoveryCursor(self.discoveryCursor),
        cap = tonumber(self.discoveryCap) or 0,
    }
end

function M:GetCaptureHealth()
    local capture = type(self.capture) == "table" and self.capture or {}
    return {
        enabled = capture.enabled == true,
        sticky = capture.sticky ~= false,
        entries = CountCaptureEntries(capture),
        serial = tonumber(capture.serial) or 0,
        cap = tonumber(capture.cap) or 0,
    }
end

function M:GetImportStageHealth()
    local stage = type(self.auraImportStage) == "table" and self.auraImportStage or nil
    return {
        staged = stage ~= nil,
        chunks = stage and (tonumber(stage.count) or tonumber(stage.chunkCount) or 0) or 0,
    }
end

function M:GetHealth()
    local catalog = self:GetCatalogHealth()
    local discovery = self:GetDiscoveryHealth()
    local capture = self:GetCaptureHealth()
    local importStage = self:GetImportStageHealth()
    return {
        version = 1,
        open = self:IsOpen(),
        ...
        frozenScope = catalog.frozenScope,
        frozenEffect = catalog.frozenEffect,
        catalogEntries = catalog.entries,
        trackedRows = catalog.trackedRows,
        liveRows = catalog.liveRows,
        runtimePolls = tonumber(self.runtimePolls) or 0,
        discoverySerial = discovery.serial,
        discoveryEntries = discovery.entries,
        captureEnabled = capture.enabled,
        captureSticky = capture.sticky,
        captureEntries = capture.entries,
        captureSerial = capture.serial,
        auraImportStaged = importStage.staged,
        auraImportChunks = importStage.chunks,
        catalog = catalog,
        discovery = discovery,
        capture = capture,
...
function M:GetDiscoveredCount(scope,effectType)
    -- Count directly from the bounded session bucket.  The overview only needs
    -- a number, so building and sorting a temporary discovery list here would
    -- add avoidable allocations every refresh. Keep the same semantics as
    -- GetDiscoveryList(): already-tracked effects are excluded.
    local bucket=DiscoveryBucket(scope,effectType)
    if type(bucket)~="table" then return 0 end
    local count=0
    for id in pairs(bucket) do
        if not S:IsTracked(scope,effectType,id) then count=count+1 end
    end
    return count
end
...
local function BuildSkillEffectsPresetTracking()
    -- Shared GameDataRegistry is the ID/query Authority. rs_skill_effects.lua
    -- remains the curated relationship source and rs_buff_ids.lua imports it
    -- once at load time; Plates no longer reaches into another data file's raw
    -- table directly. This path runs only on explicit preset import, not Tick.
    local registry=ReplicatedSuite and ReplicatedSuite.GameDataRegistry or nil
    if registry==nil or type(registry.List)~="function" then
        return nil, "共享游戏数据注册表未加载：无法导入内置实战库。"
    end
    local records=registry:List("buff")
    if type(records)~="table" or #records==0 then
        return nil, "共享 Buff ID 库为空：请检查 rs_skill_effects.lua / rs_buff_ids.lua 是否完整加载。"
    end
    local result={target={buff={},debuff={},hidden={}},player={buff={},debuff={},hidden={}}}
    for _,record in ipairs(records) do
        local id=tonumber(record and record.id)
        local name=record and tostring(record.name or "") or ""
