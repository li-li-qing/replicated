...
local function BuildPage(parent, route)
    ...
    local channelDefs={
        {id="v3_buff_track_player_buff",scope="player",category="buff",label="所选设为自身 Buff",status="自身·Buff"},
        {id="v3_buff_track_player_debuff",scope="player",category="debuff",label="所选设为自身 Debuff",status="自身·Debuff"},
        {id="v3_buff_track_target_buff",scope="target",category="buff",label="所选设为目标 Buff",status="目标·Buff"},
        {id="v3_buff_track_target_debuff",scope="target",category="debuff",label="所选设为目标 Debuff",status="目标·Debuff"},
    }
    ...
    RefreshSelectedTrackingControls=function()
        ...
        for _,def in ipairs(channelDefs) do
            ...
            if button then
                ...
                button:SetEnabled(valid);button:SetText((active and "✓ " or "")..def.label)
            end
        end
        if not row then selectedText:SetText("点击状态行选择，再指定自身 / 目标 Buff 或 Debuff。")
        elseif not valid then selectedText:SetText("所选："..tostring(row.name or row.id).." · 技能 CD 不使用状态四通道。")
        else
            local labels=SelectedChannelLabels(row)
            selectedText:SetText("所选："..tostring(row.name or row.id).." · ID "..tostring(row.id).." · "..(#labels>0 and table.concat(labels," / ") or "未追踪"))
        end
        return true
    end
    ...
    for _,def in ipairs(channelDefs) do
        local d=def
        local button=RSUI:Button({id=d.id,parent=scopeTrackRow,text=d.label,compact=true,
