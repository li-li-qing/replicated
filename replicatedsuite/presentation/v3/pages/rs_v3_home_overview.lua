...
local function Open(route)
    if type(S.UIV3.Navigate) == "function" then return S.UIV3:Navigate(route, { source="home_card" }) end
    return false, "导航不可用"
end
...
function Home:Build(parent,route)
    ...
    local specs={
        {name="Activities",id="life_activities",token="home:activities",title="活动 / 世界状态",prefix="v3_home_activity_",route="life.activities",topic="v3.activities.updated",workspace="world",position=1},
        {name="Trade",id="life_trade",token="home:trade",title="当前跑商路线",prefix="v3_home_trade_",route="life.trade",workspace="world",position=2},
        {name="Tasks",id="life_tasks",token="home:tasks",title="我的任务",prefix="v3_home_tasks_",route="life.tasks",topic="v3.tasks.updated",workspace="tasks",position=1},
        {name="Bonds",id="life_bonds",token="home:bonds",title="债券 / 居民板",prefix="v3_home_bonds_",route="life.bonds",workspace="tasks",position=2},
    }
    local taskCard
    for _,spec in ipairs(specs) do
        ...
        local card={spec=spec,feature=S.Features and S.Features[spec.name],held=false,panel=panel,body=body,badge=badge}
        ...
        if spec.name=="Trade" or spec.name=="Bonds" then
            ...
            local columns=task and {
                ...
                {id="name",title="活动",field="shortName",size="fill",minWidth=80,getTone=function(item)return item.active and "orange" or "default"end},
    ...
    local function RefreshCard(card)
        ...
        card.status:SetText("进行中 "..active.." · 显示 "..#rows.." 项 · 点击查看详情")
        return true
    end
