...
function W:_UpdateNavigation()
    -- 中文维护注释（2026-09-19，module-diagnostics-pagination-authority-1）：
    -- 原因：RU 实机出现“正文已经是 PAGE=1/2，但工具栏仍 0/0 且下一页禁用”。旧实现把
    -- 分页导航错误绑定到 DiagnosticCopyBox 的 Native 回读事务：Hub 已经生成 immutable
    -- snapshot，正文甚至已经写入编辑框，只要 CopyBox 最后一步回读/提交没有完成，snapshot
    -- 就不发布，用户因此失去其余页面。分页 Authority 本应是 Hub snapshot，不是 Native 编辑框。
    -- Authority/数据流：Hub:GetPage 成功即证明 page index 属于冻结 snapshot；窗口立刻发布
    -- snapshot/pageIndex。CopyBox 只负责“当前页显示与完整性校验”，失败仅把页码标红并在状态栏
    -- 报错，不得锁死上一页/下一页。缩短分页仍只重切同一份 report，不重新采集业务/Store。
    -- 兼容边界：分页可继续浏览不代表失败页可作为完整证据；copyPageValid=false 时仍要求用户
    -- 使用缩短分页，直到回读一致。这样既不丢第二页，也不把截断页伪装成已验证页面。
    ...
    if self.pageLabel ~= nil and type(self.pageLabel.SetTone) == "function" then
        self.pageLabel:SetTone(self.copyPageValid == false and "red" or "accent")
    end
...
function W:EnsureCreated()
    ...
    self.normalPageButton = RSUI:Button({ id = self.id .. "_normal_pages", parent = actions, text = "正常分页", compact = true,
    ...
    self.probeButton = RSUI:Button({ id = self.id .. "_aura_probe", parent = self.detailActions, text = "Buff字段", compact = true,
        slot = { size = "fixed", width = 88 }, onClick = function() return W:ProbeAuraFields() end })
    self.equipmentProbeButton = RSUI:Button({ id = self.id .. "_equipment_probe", parent = self.detailActions, text = "装备探测", compact = true,
        slot = { size = "fixed", width = 88 }, onClick = function() return W:ProbeEquipmentFields() end })
    ...
    RSUI:Text({ id = self.id .. "_detail_hint", parent = self.detailActions, text = "Buff字段查状态结构；装备探测查远程槽。两者都会输出可复制聊天证据。",
    ...
    self.normalPageButton.onClick = function() return W:RestoreNormalPages() end
...
function W:Open(moduleId)
    ...
    if workspace then SetText(self.stateText,workspace:DescribeModule(moduleId).." · 详细诊断需手动生成") end
...
function W:_PresentSnapshot(snapshot, index)
    ...
    -- Hub snapshot/page 是不可变的报告分页 Authority。只要 GetPage 成功，就先发布导航状态；
    -- Native CopyBox 写入/回读属于下一层展示事务。这样即使 RU 编辑框回读异常，用户仍能
    -- 访问冻结报告的其余页面，再用“缩短分页”解决当前页复制完整性，而不是卡在 0/0。
    self.snapshot, self.pageIndex = snapshot, index
...
function W:RestoreNormalPages()
    ...
    self.copyBox:Deactivate("restore_normal_pages")
    local ok, snapshot, err = pcall(Hub.Repage, Hub, source, NORMAL_PAGE_CAPACITY)
    if not ok or snapshot == nil then return self:_ReportFailure(ok and err or snapshot) end
    self.pendingSnapshot = snapshot
    local shown, detail = self:_PresentSnapshot(snapshot, 1)
...
function W:ProbeEquipmentFields()
    if self.moduleId ~= "combat_buff_display" then return false, "当前模块不支持装备字段探测" end
    local feature = S.Features and S.Features.BuffDisplay
    local command = feature and feature.Commands and feature.Commands.ProbeEquipmentFields
    if type(command) ~= "function" then return self:_ReportFailure("装备字段探测不可用") end
    local ok, accepted, detail = pcall(command, feature.Commands)
    local result = ok and detail or accepted
    local text = tostring(result or "无返回信息")
    if #text > 8192 then text = text:sub(1, 8192) .. "[TRUNCATED]" end
    self.lastEquipmentProbe = { success = ok and accepted == true, detail = text,
        capturedAt = type(S.NowMs) == "function" and S.NowMs() or 0 }
    if self.surface and type(self.surface.SetStatus) == "function" then
        self.surface:SetStatus((ok and accepted == true and "装备探测完成" or "装备探测失败")
            .. "；聊天已输出单条远程槽证据，也可再点生成诊断收录。", ok and accepted == true and "accent" or "red")
    end
    return ok and accepted == true, result
end
...
if type(Hub.RegisterProvider) == "function" then
    ...
    Hub:RegisterProvider("combat_buff_display", "explicit_equipment_probe", function()
        return W.lastEquipmentProbe or { sampled = false, reason = "only_runs_on_explicit_equipment_probe_button" }
    end)
end
