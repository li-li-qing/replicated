...
function M:Finish(bar, page)
    ...
    local action = type(button.GetOnClick) == "function" and button:GetOnClick() or button.onClick or button.spec.onClick
        ...
        if not ok or result == false then
            ...
            if S.DiagnosticsManager and type(S.DiagnosticsManager.Error) == "function" then
                S.DiagnosticsManager:Error("ui_v3", "MODULE_CONTROL_REJECTED", "模块启动/关闭操作未完成",
                    { moduleId = bar.featureId, route = bar.route, error = tostring(reason or "action returned false") })
            end
            return false, reason
        end
        return result, detail
    end
    if type(button.SetOnClick) == "function" then button:SetOnClick(Invoke)
    else button.onClick, button.spec.onClick = Invoke, Invoke end
    self:Refresh(bar)
    return true
end
