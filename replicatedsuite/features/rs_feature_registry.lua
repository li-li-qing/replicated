...
Add("life_trade", "life.trade", "跑商", "life", 20, "路线、多货物与实时货率；材料身份/数量可读，材料报价与利润改为独立显式询价后再接回。", {
    ...
    apiReadiness = "official_mixed", apiPolicy = "on_demand_server_query", currentImplementation = "路线/区域/服务器货率 + bounded 材料 itemType/数量投影；普通 Refresh 不调用拍卖报价；QuoteMaterial/QuotePendingMaterials 显式进入共享 PriceQuoteQueueV3，报价完成后只重建受影响 Trade 行的材料成本/利润；生产地区 API 失败或空响应可使用 sealed Zone 候选，但 GetSpecialtyRatioBetween 仍是路线 Authority", remainingCapability = "GetLowestPrice 返回形态、RU 生产/可售地区 payload 与实际价格一致性仍需实机验证；current/full ratio 与 commerce skill mode 尚未迁移", evidence = "V3 Trade Authority + SPECIALTY_RATIO_BETWEEN_INFO; quote fan-out removed in M1.16.0.18.39; explicit Trade quote workflow .18.82/.18.93",
})
...
Add("tools_bag", "tools.bag_organizer", "整理背包", "tools", 10, "背包/仓库整理、黑名单与按类别有界批量移动。", {
    ...
    capabilities = { "category_batch", "scheduler_queue", "window_commands", "native_window_quick_take_put", "blacklist_filter", "read_verify_stop", "dynamic_source_resolution" },
    scheduler = "Shared Scheduler serializes bounded category moves and native-window quick take/put; quick/category tasks are mutually exclusive; one move per scheduled step; source slot is re-resolved from stable itemType/category intent before each write; no per-frame polling",
    ...
    apiPolicy = "read_plus_explicit_move", evidence = "Active V3 uses ADDON:GetContentMainScriptPosVis only for bounded geometry observation; overlay is an independent RSUI Presenter, not Native reparenting. Quick take/put and category batch are mutually exclusive, serialize at 250ms, keep stable itemType/category intent instead of slot identity, re-resolve the live source slot before every write, and verify ambiguous post-move compaction by bounded source-population decrease; they stop on storage close/change or feature disable. RU visual anchoring and move timing still require Fresh Reload proof.",
})
