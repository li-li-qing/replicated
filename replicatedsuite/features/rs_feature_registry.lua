...
Add("life_trade", "life.trade", "跑商", "life", 20, "路线、多货物与实时货率；材料身份/数量可读，材料报价与利润改为独立显式询价后再接回。", {
    ...
    apiReadiness = "official_mixed", apiPolicy = "on_demand_server_query", currentImplementation = "路线/区域/服务器实时货率 + 满货率 130% 本地对比模式（持久化）+ 经商熟练度只读观察；熟练度精确 RU 售价倍率未验证前明确不计入估价。路线收藏最多 12 条并由 life.trade 单 Store 持久化，主页面与 life.trade HUD 共用收藏/排序命令；选中贸易品复用 TradeDetailFloatingV3，详情仅在可见期持有独立 Consumer，并通过 QuoteRowMaterials 显式询价当前材料。普通 Refresh 不产生 Auction fan-out。", remainingCapability = "GetLowestPrice 返回形态、RU 生产/可售地区 payload 与实际价格一致性仍需实机验证；经商熟练度对最终货物收益的精确 RU 公式仍待实证，不能沿用旧版猜测倍率；自动制作台刷新/叛乱记录仍缺安全事件证据", evidence = "V3 Trade Authority + SPECIALTY_RATIO_BETWEEN_INFO + official X2Ability actability list; .18.119 current/full ratio contract; .18.120 bounded favorites + shared TradeDetailFloatingV3 + explicit selected-row material quote",
})
