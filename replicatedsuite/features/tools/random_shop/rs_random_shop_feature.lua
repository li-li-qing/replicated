------------------------------------------------------------------------
-- Replicated Suite V3 - Random Shop Feature Lifecycle
------------------------------------------------------------------------
...
function F:ReconcileDemand(_, before, after)
    if (tonumber(before and before.count) or 0) <= 0 and (tonumber(after and after.count) or 0) > 0 then
        return self.Authority:Refresh("random_shop_consumer_acquire")
    end
    return true
end
...
function F:AcquireConsumer(token)
    if self.enabled ~= true then return false, "random shop feature disabled" end
...
function F:Disable(reason)
    local cleared, clearErr = self.Demand:Clear(reason or "random_shop_feature_disable")
...
function F:Refresh(reason)
    ...
    return self.Authority:Refresh(reason or "random_shop_refresh")
end
