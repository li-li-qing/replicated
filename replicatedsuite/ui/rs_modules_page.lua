...
function S.ModulesPage.Create(parent)
    local page={key="modules",root=S.UI:CreatePanel(parent,"modules_page",0,0,100,100,"soft"),rows={},searchRows={}}
    ...
    page.title=S.UI:CreateLabel(page.root,"modules_title","功能开关 / 方案",12,8,300,28,16,nil,ALIGN_LEFT)
    page.note=S.UI:CreateLabel(page.root,"modules_note","这里只管理功能生命周期与方案；具体功能参数回到对应功能页，HUD 外观统一在“悬浮窗管理”。",12,36,760,22,9,"muted",ALIGN_LEFT)
    ...
    for i,d in ipairs(modules) do
        ...
        page.rows[i]={id=d.id,category=d.category,label=label,state=state,toggle=toggle,settings=settings,retry=retry}
    ...
    function page:Refresh()
        ...
        -- System overview is derived from the two existing authorities only.
        -- Do not maintain a second module/HUD state cache on this page.
        local moduleTotal, moduleEnabled, moduleFaulted = 0, 0, 0
        for _, d in ipairs(S.ModuleManager and S.ModuleManager:List(false) or {}) do
            moduleTotal = moduleTotal + 1
            if d.enabled == true then moduleEnabled = moduleEnabled + 1 end
            if d.state == "Faulted" then moduleFaulted = moduleFaulted + 1 end
        end
        local hud = S.HudManager and S.HudManager:GetOverview() or {}
        local hudText = tostring(hud.effectiveVisible or 0) .. "/" .. tostring(hud.total or 0)
        self.note:SetText("Runtime 模块 " .. tostring(moduleEnabled) .. "/" .. tostring(moduleTotal)
            .. (moduleFaulted > 0 and (" · 故障 " .. tostring(moduleFaulted)) or "")
            .. " · HUD显示 " .. hudText .. "；关闭模块不会清空配置、统计或 HUD 长期偏好。")
        ...
        for _,r in ipairs(self.rows) do
            ...
            if d then
                local categoryLabel = S.UICatalog and type(S.UICatalog.GetModuleCategoryLabel)=="function" and S.UICatalog:GetModuleCategoryLabel(d.category) or tostring(d.category or "")
                r.label:SetText(categoryLabel .. "｜" .. tostring(d.name or r.id))
    ...
    function page:ApplyLayout(spec)
        ...
        local toggleW,settingsW,retryW=58*sc,54*sc,42*sc
        local actionsW=toggleW+settingsW+retryW+actionGap*2
        ...
        for i,r in ipairs(self.rows) do
            ...
            for _,entry in ipairs({{r.toggle,toggleW},{r.settings,settingsW},{r.retry,retryW}}) do
