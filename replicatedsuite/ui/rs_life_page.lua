...
function P.Create(parent)
    ...
    -- Legacy rollback-only dashboard: keep a small recovery/tool card, but do
    -- not reintroduce the removed navigation Favorites concept.
    page.cards[8] = CreateListCard(page.root, "life_quick", "备用工具", 12, "工具", function() S.UI:ShowPage("quick") end, function(row)
    ...
    function page:BuildQuick()
        ...
        rows[#rows + 1] = { name = "实用工具", status = "工具", tone = "blue", actionPage = "quick" }
        rows[#rows + 1] = { name = "悬浮窗管理", status = "HUD", tone = "blue", actionPage = "hud" }
        rows[#rows + 1] = { name = "诊断与维护", status = "系统", tone = "muted", actionPage = "diagnostics" }
        return rows
    end
