...
-- Authority：本控制器只拥有“诊断报告只读文本 + 复制交互”；正文 Authority 仍在 Diagnostics，
...
UI.DiagnosticCopyBoxContractVersion = 1
...
function UI:CreateDiagnosticCopyBox(spec)
    ...
    local readOnly, readOnlyErr = AcceptedCall(edit, "SetReadOnly", true)
    if readOnly ~= true then
        if type(self.RetireInputWidget) == "function" then pcall(self.RetireInputWidget, self, edit, owner, "diagnostic_readonly_failed") end
        return nil, readOnlyErr or "diagnostic readonly unavailable"
    end
