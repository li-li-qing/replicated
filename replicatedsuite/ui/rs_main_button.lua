...
function B.Create()
    ...
    S.UI:SafeHandler(button,"OnClick",function()
        if button.rsIgnoreClick then button.rsIgnoreClick=false; return end
        if S.UIHostManager ~= nil and type(S.UIHostManager.Toggle) == "function" then S.UIHostManager:Toggle("legacy") else S.UI:ToggleMain() end
    end,"entry:click")
