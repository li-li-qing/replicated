...
function PAGES.CreateHealer(parent)
    ...
    -- UI Phase B sample: the common Healer settings now use the v2 bound-field
    -- layer.  Binding preview delegates to the Healer SettingsModel through the
    -- Presenter facade; the Domain remains the final write/persistence Authority.
    page.v2BasicFields={}
    local function CreateHealerSettingBinding(key,fallback)
        local previewReady,previewOk,previewValue,previewError=false,false,nil,nil
        return S.UI:CreateSettingBinding({
            id="healer.basic."..tostring(key),
            get=function() return Get(key,fallback) end,
            normalize=function(value,previous)
                local h=H()
                previewReady,previewOk,previewValue,previewError=true,false,nil,"治疗模块未初始化"
                if h and h.PreviewSuiteSetting then
                    previewOk,previewValue,previewError=h:PreviewSuiteSetting(key,value)
                    if previewOk==true then return previewValue end
                end
                return value
            end,
            validate=function(value)
                if previewReady==true then
                    local ok,err=previewOk,previewError
                    previewReady=false
                    return ok==true,err
                end
                local h=H()
                if not h or not h.PreviewSuiteSetting then return false,"治疗模块未初始化" end
                local ok,_,err=h:PreviewSuiteSetting(key,value)
                return ok==true,err
            end,
            set=function(value) return Set(key,value) end,
        })
    end
    local function RegisterBasicField(field)
        if field==nil then return nil end
        page.v2BasicFields[#page.v2BasicFields+1]=field
        page:RegisterWidget("basic",field.root)
        return field
    end
    for _,a in ipairs({
        ...
        local item=a
        RegisterBasicField(S.UI:CreateNumericFieldV2(page.root,"healer_basic_"..item[1],{
            label=item[3],min=item[4],max=item[5],step=item[6],integer=true,unit=item[8],
            binding=CreateHealerSettingBinding(item[2],item[7]),
        }))
    end
    RegisterBasicField(S.UI:CreateToggleFieldV2(page.root,"healer_basic_prox",{
        label="治疗范围底色",onText="已开启",offText="已关闭",binding=CreateHealerSettingBinding("proximityMode",true),
    }))
    RegisterBasicField(S.UI:CreateChoiceFieldV2(page.root,"healer_basic_hcurve",{
        label="血量曲线",values={{value=1,label="线性"},{value=2,label="低血量加速"}},binding=CreateHealerSettingBinding("healthCurveMode",2),
    }))
    RegisterBasicField(S.UI:CreateChoiceFieldV2(page.root,"healer_basic_haccel",{
        label="血量加速",values={{value=1,label="柔和"},{value=2,label="标准"},{value=3,label="强烈"}},binding=CreateHealerSettingBinding("healthAccelMode",2),
    }))
    RegisterBasicField(S.UI:CreateChoiceFieldV2(page.root,"healer_basic_dcurve",{
        label="距离曲线",values={{value=1,label="线性"},{value=2,label="边缘惩罚"}},binding=CreateHealerSettingBinding("distanceCurveMode",2),
    }))
    ...
    function page:Refresh()
        ...
        if self.activeSection=="basic" then
            for _,field in ipairs(self.v2BasicFields or {}) do if field and field.Render then field:Render() end end
        end
    ...
    function page:ApplyLayout(spec)
        local sc,pad,full,top=self:ApplyBase(spec)
        if self.activeSection=="basic" then
            if S.UI.ComponentsV2 and S.UI.ComponentsV2.LayoutFieldGrid then
                S.UI.ComponentsV2:LayoutFieldGrid(self.root,self.v2BasicFields,{
                    x=pad,y=top,width=full,minCellWidth=250*sc,maxColumns=2,
                    cellHeight=52*sc,gapX=8*sc,gapY=8*sc,owner=self.root.rsUiOwner,
                })
            end
