...
local buffAnchor = CreateEmptyWindow("targetDebuffTrackerSelfHiddenAnchor", "UIParent")
...
function buffAnchor:OnUpdate()
	...
	if shared.GetUiState().self.hidden then
		local settings = shared.GetIconSettings("self", "hidden")
		local buffCounter = 0
		local buffCount = X2Unit:UnitHiddenBuffCount("player")
		for i = 1, buffCount do
			local buff = X2Unit:UnitHiddenBuffTooltip("player", i)
			local buffExtra = X2Unit:UnitHiddenBuff("player", i)
