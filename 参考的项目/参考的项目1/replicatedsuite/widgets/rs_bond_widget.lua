...
function S.BondWidget.Create()
    ...
    local rows={}
    for i=1,widget.pageSize do
        rows[i]={
            area=S.UI:CreateLabel(win,"bond_widget_area_"..i,"",12,0,112,22,9,"muted",ALIGN_LEFT),
            detail=S.UI:CreateLabel(win,"bond_widget_detail_"..i,"",126,0,365,22,9,nil,ALIGN_LEFT),
            done=S.UI:CreateLabel(win,"bond_widget_done_"..i,"",500,0,35,22,12,"muted",ALIGN_CENTER),
        }
    ...
    function widget:Refresh()
        ...
        for i,w in ipairs(rows) do
            local index=(self.page-1)*self.pageSize+i; local r=entries[index]; local show=r~=nil and S.State.ui.widgets.bond.mode=="standard"
            w.area:Show(show); w.detail:Show(show); w.done:Show(show)
            if show then
                w.area:SetText("["..tostring(r.continentLabel or "--").."] "..tostring(r.material or ""))
                w.detail:SetText(tostring(r.text or "--")); w.done:SetText(tostring(r.status or "?")); S.Theme:SetLabelTone(w.done,r.tone)
            end
        end
    ...
    widget.OnLayout=function(self,width,height,titleHeight,mode)
        ...
        if standard then
            ...
            hint:SetExtent(width-24*scale,20*scale); S.UI:SetAnchor(hint,win,12*scale,titleHeight+32*scale)
            local rowStart=titleHeight+58*scale; local footerH=31*scale; local rowH=29*scale
            ...
            for i,w in ipairs(rows) do
                ...
                w.area:SetExtent(112*scale,22*scale); w.detail:SetExtent(math.max(160*scale,width-182*scale),22*scale); w.done:SetExtent(30*scale,22*scale)
                S.UI:SetAnchor(w.area,win,12*scale,y); S.UI:SetAnchor(w.detail,win,126*scale,y); S.UI:SetAnchor(w.done,win,width-42*scale,y)
                if i>maxFit then w.area:Show(false); w.detail:Show(false); w.done:Show(false) end
            end
