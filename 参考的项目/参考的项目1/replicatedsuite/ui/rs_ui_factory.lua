...
function UIX:Register(id, widget)
    if id ~= nil and widget ~= nil then
        ...
        if previous ~= nil and previous ~= widget then
            -- A duplicated logical control id makes later Refresh/Layout code
            -- address whichever widget happened to register last. Keep runtime
            -- compatible (last writer still wins), but surface the collision so
            -- it cannot remain an invisible UI/button routing bug.
            S.WarnOnce("ui_control_duplicate:" .. key, "UI 控件 ID 重复：" .. key)
        end
...
function UIX:SafeHandler(widget, eventName, fn, label)
    ...
    local function handler(...)
        ...
        if not ok then
            S.SafeChat("UI错误 " .. tostring(label or eventName) .. "：" .. tostring(err))
            return nil
        end
    ...
    return ok == true and bindResult ~= false
end
...
function UIX:CreateManagedWindow(spec)
    ...
    if id == "" or type(CreateEmptyWindow) ~= "function" or S.State == nil then return nil end
    ...
    local window = CreateEmptyWindow(S.PhysicalId("dialog_" .. id), "UIParent")
...
function UIX:CreatePanel(parent, id, x, y, width, height, kind, opts)
    local panel = UIParent:CreateWidget("emptywidget", S.PhysicalId(id), parent)
...
function UIX:CreateLabel(parent, id, text, x, y, width, height, fontSize, tone, align, shadow)
    ...
    S.Theme:StyleLabel(label, fontSize, tone, align, useShadow)
...
function UIX:CreateEditBox(parent, id, x, y, width, height, maxLength)
    ...
    local widgetType = UOT_X2_EDITBOX or UOT_EDITBOX or (OBJECT_TYPE and OBJECT_TYPE.X2_EDITBOX) or (OBJECT_TYPE and OBJECT_TYPE.EDITBOX)
    if widgetType == nil then return nil end
    local ok, edit = pcall(function() return parent:CreateChildWidgetByType(widgetType, S.PhysicalId(id), 0, true) end)
    local fallbackType = UOT_EDITBOX or (OBJECT_TYPE and OBJECT_TYPE.EDITBOX)
    if (not ok or edit == nil) and fallbackType ~= nil and widgetType ~= fallbackType then
        ok, edit = pcall(function() return parent:CreateChildWidgetByType(fallbackType, S.PhysicalId(id), 0, true) end)
    end
    if not ok or edit == nil then return nil end
...
function UIX:CreateMultiEditBox(parent, id, x, y, width, height, maxLength)
    if parent == nil or type(parent.CreateChildWidgetByType) ~= "function" then return nil end
    local widgetType = UOT_EDITBOX_MULTILINE or (OBJECT_TYPE and OBJECT_TYPE.EDITBOX_MULTILINE)
    if widgetType == nil then return nil end
    local ok, edit = pcall(function() return parent:CreateChildWidgetByType(widgetType, S.PhysicalId(id), 0, true) end)
    if not ok or edit == nil then return nil end
    local configured = pcall(function()
...
function UIX:CreateSlider(parent, id, x, y, width, height, minimum, maximum, step, value)
    ...
    local ok, slider = pcall(function()
        return parent:CreateChildWidget("emptywidget", S.PhysicalId(id), 0, true)
    end)
    if not ok or slider == nil then return nil end
    ...
    local configured = pcall(function()
        ...
        local thumb = slider:CreateChildWidget("emptywidget", S.PhysicalId(id .. "_thumb"), 0, true)
        ...
        local drag = slider:CreateChildWidget("emptywidget", S.PhysicalId(id .. "_drag"), 0, true)
...
function UIX:CreateButton(parent, id, text, x, y, width, height, fontSize, active, useGradient)
    ...
    S.Theme:StyleButton(button, width, height, fontSize, active, useGradient)
