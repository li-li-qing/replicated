...
function UI:CreateNumericSettingControl(parent, id, options)
    ...
    function row:SetValue(value, emit, final, source)
        ...
        local change = options.onChanged or (options.binding and options.binding.Set)
        if emit == true and type(change) == "function" then
            local ok, accepted = xpcall(function() return change(nextValue, final == true, tostring(source or "program"), previous) end, S.SafeTraceback)
