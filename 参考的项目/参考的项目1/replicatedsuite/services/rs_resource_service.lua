...
function R:EnsureDate()
    ...
    if current == "unknown" then
        ...
        if verdict == true then
            ...
            -- Temp diagnostics (2026-08-24, remove after root-cause confirmed):
            -- shows the exact persisted block a UI reload actually recovered.
            S.SafeChat("[资源诊断] 已从存档恢复：date=" .. tostring(current)
                .. " gold=" .. tostring(tonumber(counters.gold) or 0)
                .. " honor=" .. tostring(tonumber(counters.honor) or 0)
                .. " vocation=" .. tostring(tonumber(counters.vocation) or 0)
                .. " exp=" .. tostring(tonumber(counters.exp) or 0))
        elseif verdict == "empty" then
            ...
            -- Temp diagnostics (2026-08-24, remove after root-cause confirmed).
            S.SafeChat("[资源诊断] 主存档与独立计数存档均无每日计数，已按今天初始化"
                .. (seeded == true and "并已写入独立存档。" or "（独立存档写入失败！loadFailed="
                    .. tostring(S.Storage ~= nil and S.Storage.loadFailed or "?") .. "）"))
            return
        else
            -- Could not confirm what the disk holds. Never zero, never write;
            -- re-enter this path shortly (bounded by resource_debounce).
            S.WarnOnce("resource_disk_unreadable",
                "[资源诊断] 重载后无法读取存档计数（ADDON:LoadData 返回空/失败），已保持原值不覆盖，稍后自动重试。")
    ...
    S.Storage:RequestSave(0)
    -- Temp diagnostics (2026-08-24, remove after root-cause confirmed).
    S.SafeChat("[资源诊断] 判定跨天（旧=" .. tostring(current) .. " 新=" .. tostring(key) .. "），今日收益重置。")
end
...
function R:Start()
    ...
    -- Temp diagnostics (2026-08-24, remove after root-cause confirmed): one-shot
    -- confirmation that the anti-reset build is loaded AND the current counter
    -- state that was restored from the dedicated save key.
    local probe = S.State and S.State.dailyCounters or {}
    S.SafeChat("[资源诊断] 计数模块已加载（v2 防清零修复）；当前 date=" .. tostring(probe.dateKey or "unknown")
        .. " gold=" .. tostring(tonumber(probe.gold) or 0)
        .. " honor=" .. tostring(tonumber(probe.honor) or 0)
        .. " vocation=" .. tostring(tonumber(probe.vocation) or 0)
        .. " exp=" .. tostring(tonumber(probe.exp) or 0))
    self:Refresh()
end
