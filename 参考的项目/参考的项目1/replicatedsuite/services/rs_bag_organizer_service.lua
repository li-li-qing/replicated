...
function O:RefreshViews()
    ...
    if page ~= nil and type(page.Refresh) == "function" then
        ...
        if not ok and S.DiagnosticsManager and type(S.DiagnosticsManager.Record) == "function" then
...
function O:ApplyFloatingLayout()
    local floating = self.floating
    if type(floating) ~= "table" or floating.panel == nil then return end
    local panel = floating.panel

    local panelW, panelH = 100, 34
    panel:SetExtent(panelW, panelH)
    if panel.RemoveAllAnchors then panel:RemoveAllAnchors() end

    local context = S.Layout:GetContext()
    local edge = math.max(6, tonumber(context.safeTop) or 12)
    local logicalW = tonumber(context.logicalWidth) or 1024
    local logicalH = tonumber(context.logicalHeight) or 768
    local rect = self:ResolveBagAnchorRect()

    if type(rect) == "table" then
        local targetX = rect.x + math.max(0, (rect.width - panelW) / 2)
        local targetY = rect.y - panelH - 4
        local placement = "above"

        -- 1024x768 / large UI scale: when there is genuinely no room above the
        -- bag, keep the strip inside the bag's top edge instead of off-screen.
        ...
        if S.Layout ~= nil and type(S.Layout.ClampTopLeft) == "function" then
            targetX, targetY = S.Layout:ClampTopLeft(targetX, targetY, panelW, panelH, { edge = edge })
        else
            targetX = math.max(edge, math.min(targetX, math.max(edge, logicalW - edge - panelW)))
            targetY = math.max(edge, math.min(targetY, math.max(edge, logicalH - edge - panelH)))
        end
        panel:AddAnchor("TOPLEFT", "UIParent", math.floor(targetX + 0.5), math.floor(targetY + 0.5))
        self.floatingPlacement = placement
    else
        -- Geometry failure must be obvious and deterministic. Keep the controls
        -- available while diagnostics reports 'unresolved', rather than putting
        -- them at (0,0) and pretending that is the bag position.
        local safeX = math.max(edge, math.floor((logicalW - panelW) / 2))
        local safeY = edge
        panel:AddAnchor("TOPLEFT", "UIParent", safeX, safeY)
        self.floatingPlacement = "safe-top-unresolved"
    end

    S.UI:SetAnchor(floating.withdraw, panel, 4, 4); floating.withdraw:SetExtent(44, 26)
    S.UI:SetAnchor(floating.deposit, panel, 52, 4); floating.deposit:SetExtent(44, 26)
end

function O:EnsureFloatingUi()
    if self.floating ~= nil and self.floating.panel ~= nil then return true end
    local bag = self:GetBagContent()
    if type(CreateEmptyWindow) ~= "function" then
        self.lastMessage = "快捷按钮创建失败：CreateEmptyWindow 不可用"
        return false
    end

    -- Use a real system-layer window rather than an UIParent emptywidget. Native
    -- bag/bank windows can draw above ordinary widgets even after Raise(), which
    -- made the previous quick buttons logically visible but visually hidden.
    local panel = CreateEmptyWindow(S.PhysicalId("bag_organizer_floating"), "UIParent")
    if panel == nil then
        self.lastMessage = "快捷按钮创建失败：窗口创建失败"
        return false
    end
    panel:SetExtent(100, 34)
    ...
    self.floating = { panel = panel, withdraw = withdraw, deposit = deposit, bag = bag }
    S.UI:SafeHandler(withdraw, "OnClick", function() O:Begin("withdraw") end, "bag_organizer:floating_withdraw")
    S.UI:SafeHandler(deposit, "OnClick", function() O:Begin("deposit") end, "bag_organizer:floating_deposit")
    panel:Show(false)
    self:ApplyFloatingLayout()
    return true
end

function O:RefreshFloating()
    if self.floating == nil or self.floating.panel == nil then self:EnsureFloatingUi() end
    local floating = self.floating
    ...
    -- Product rule: either bank or coffer interaction enables the quick bar.
    -- We do not require the bag visibility proxy because that proxy is known to
    -- be unreliable on this RU client; placement has independent resolution.
    local shouldShow = self.started == true
        and S.State.settings.bagOrganizerShowBagButtons == true
        and storageVisible == true
    floating.panel:Show(shouldShow)
    floating.withdraw:SetText(self.busy and self.direction == "withdraw" and "取…" or "取")
    floating.deposit:SetText(self.busy and self.direction == "deposit" and "放…" or "放")
    if floating.withdraw.Enable then floating.withdraw:Enable(self.busy ~= true) end
    if floating.deposit.Enable then floating.deposit:Enable(self.busy ~= true) end
    if shouldShow and floating.panel.Raise then pcall(function() floating.panel:Raise() end) end
end
...
function O:WatchFloatingVisibility()
    ...
    if self.floating == nil or self.floating.panel == nil
        or bankVisible ~= self.lastBankVisible or cofferVisible ~= self.lastCofferVisible
        or bagVisible ~= self.lastBagVisible then
        self:RefreshFloating()
    elseif storageKind ~= nil then
        self:ApplyFloatingLayout()
        if self.floating and self.floating.panel and self.floating.panel.Raise then
            pcall(function() self.floating.panel:Raise() end)
        end
    end
end
...
function O:Stop()
    ...
    self.lastBagAnchorRect = nil
    self.floatingPlacement = "hidden"
    if self.floating and self.floating.panel then self.floating.panel:Show(false) end
    return true
end
