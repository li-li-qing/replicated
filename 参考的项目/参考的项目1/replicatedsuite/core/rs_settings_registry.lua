...
function R:Open(id)
    ...
    if def.Page ~= nil and S.UI ~= nil and type(S.UI.ShowPage) == "function" then
        return S.UI:ShowPage(def.Page) == true
    end
...
local native = {
    {"suite:scale","界面缩放","缩放 大小 UI","设置"},
    {"suite:font","全局字体","字体 字号 HUD","设置"},
    {"suite:opacity","主面板透明度","透明度 背景","设置"},
    {"suite:hud","HUD管理","悬浮窗 布局 位置 缩放 透明度 字体","HUD"},
    {"suite:event","活动时间","活动 倒计时 鲸鱼 烛台 庭院","生活"},
    {"suite:trade","跑商货率","跑商 货率 材料 价格 拍卖","生活"},
    {"suite:bonds","债券居民板","债券 居民板 20 60 100 原大陆","生活"},
    {"suite:fishing","钓鱼辅助","钓鱼 按键 R","生活"},
    {"suite:diagnostics","诊断","错误 API Capability Runtime 模块","诊断"},
}
for _, row in ipairs(native) do
    R:Register({ Id=row[1], Title=row[2], Keywords=row[3], Category=row[4], Page=(row[4]=="HUD" and "hud" or row[4]=="诊断" and "diagnostics" or row[4]=="设置" and "settings" or "life") })
end
