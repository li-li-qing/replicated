...
local function OpenSuitePage(pageId, target)
    if S.UI ~= nil and type(S.UI.ShowPage) == "function" then
        if S.UI:ShowPage(pageId) ~= true then return false end
        if target ~= nil and S.ProfessionalPages ~= nil and type(S.ProfessionalPages.SetSection) == "function" then
            ...
            elseif pageId == "healer" then
                -- The user-facing state workflow is now the simple BUFF
                -- condition-group page. Redirect historical observer/rule links
                -- there instead of opening hidden legacy editors.
                local map = {
            ...
            elseif pageId == "plates" then
                -- Keep old settings-search/bookmark targets compatible with the
                -- BUFF UI2 section model.
                local map = {
            ...
            elseif pageId == "gear" then
                -- Gear is a single workspace; all historical destinations land
                -- on the same visible editing surface.
