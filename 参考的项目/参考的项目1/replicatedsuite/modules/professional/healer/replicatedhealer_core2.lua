...
function HideMarkerParts(marker)
	for _, part in ipairs(marker.parts) do
		part:SetVisible(false)
	end
	marker.infoBg:SetVisible(false)
end

function AnchorMarkerPart(part, parent, x, y, width, height)
	part:RemoveAllAnchors()
	part:AddAnchor("TOPLEFT", parent, x, y)
	part:SetExtent(math.max(1, width), math.max(1, height))
	part:SetVisible(true)
end

function LayoutHeadMarker(marker, candidate)
	local size = state.headSizes[candidate.level]
	local showExtra = state.showHeadName or state.showHeadDistance or state.showHeadScore
	local infoWidth = showExtra and 130 or 66
	local markerWidth = size + 4 + infoWidth
	marker.window:SetExtent(markerWidth, size)
	HideMarkerParts(marker)
	marker.rankLabel:RemoveAllAnchors()
	marker.rankLabel:AddAnchor("TOPLEFT", marker.window, 0, 0)
	marker.rankLabel:SetExtent(size, size)
	marker.rankLabel.style:SetFontSize(math.max(9, math.floor(size * 0.38)))
	marker.infoLabel:RemoveAllAnchors()
	marker.infoLabel:AddAnchor("TOPLEFT", marker.window, size + 4, 0)
	marker.infoLabel:SetExtent(infoWidth, size)
	marker.infoLabel.style:SetFontSize(math.max(8, math.floor(size * 0.28)))
...
function RefreshHeadMarkers()
	for rank = 1, MAX_HEAD_MARKERS do
		local marker = headMarkers[rank]
		if marker ~= nil then
			marker.window:Show(false)
		end
	end
	if not state.enabled then
		return
	end
	local count = math.min(state.headMarkerCount, #recommendations)
	for rank = 1, count do
		...
		if screenX ~= nil and screenY ~= nil and screenZ ~= nil and screenZ > 0 then
			...
			for _, part in ipairs(marker.parts) do
				part:SetColor(candidate.color.r, candidate.color.g, candidate.color.b, alpha)
			end
			marker.infoBg:SetColor(0.02, 0.025, 0.035, 0.72)
			marker.rankLabel:SetText(tostring(candidate.rank))
			local infoParts = { string.format("%.0f%%", candidate.healthPercent) }
			if state.showHeadName then infoParts[#infoParts + 1] = candidate.name end
			if state.showHeadDistance then infoParts[#infoParts + 1] = string.format("%.0fm", candidate.distance) end
			if state.showHeadScore then infoParts[#infoParts + 1] = string.format("%.0f分", candidate.finalScore) end
			marker.infoLabel:SetText(table.concat(infoParts, " "))
			marker.window:RemoveAllAnchors()
			marker.window:AddAnchor(
				"TOPLEFT",
				"UIParent",
				math.floor(screenX - width / 2),
				math.floor(screenY - height - 38)
			)
			marker.window:Show(true)
		end
	end
end
...
local function EnsureRaidOverlayZOrder(overlay, raiseNow)
...
function ApplyCalibrationMode()
	...
	for sectionIndex = 1, 4 do
		...
		overlay.window:Show(calibrationVisible or (not calibrationMode and state.enabled and (rosterMode == "raid" or rosterMode == "coraid")))
...
function RefreshRaidHighlights()
	...
	for sectionIndex = 1, 4 do
		...
		if overlay.window:IsVisible() ~= showOverlay then
			overlay.window:Show(showOverlay)
			if showOverlay then EnsureRaidOverlayZOrder(overlay, true) end
		end
		for localIndex = 1, MEMBERS_PER_SECTION do
			overlay.slots[localIndex]:SetVisible(calibrationVisible)
			overlay.rankLabels[localIndex]:Show(false)
			if calibrationVisible then
				...
				overlay.slots[localIndex]:SetColor(0.16, 0.52, 1.00, 0.08)
			end
		end
	end
	if not state.enabled then
	...
	for _, candidate in ipairs(recommendations) do
		...
		if canRender then
			...
			overlay.slots[localIndex]:SetColor(candidate.color.r, candidate.color.g, candidate.color.b, alpha)
			overlay.slots[localIndex]:SetVisible(true)
			if not calibrationMode and state.showRaidRanks and candidate.rank <= state.raidRankCount then
				overlay.rankLabels[localIndex]:SetText(tostring(candidate.rank))
				overlay.rankLabels[localIndex].style:SetColor(1, 1, 1, state.raidRankAlpha)
				overlay.rankLabels[localIndex]:Show(true)
			end
		end
	end
			...
			if snapshot ~= nil and snapshot.distance ~= nil and snapshot.distance <= state.maxDistance
				...
				if overlay ~= nil and not overlay.slots[localIndex]:IsVisible() then
					...
					if color ~= nil and (state.proximityMode or visualPriority > 1) then
						overlay.slots[localIndex]:SetColor(color.r, color.g, color.b, color.a)
						overlay.slots[localIndex]:SetVisible(true)
					end
				end
			end
...
roleEnabledButton = CreateCycleControl(4, "replicatedHealerV2RoleEnabled", "职责评分", 90,
	function() state.roleScoringEnabled = not state.roleScoringEnabled end)
...
for row = 1, buffObserverPageSize do
	...
	item.add:SetHandler("OnClick", function()
		...
		if status ~= nil then AddTrackedBuffFromStatus(status) RefreshBuffObserver(false) end
	end)
...
function GetSortedObservedStatuses(member, scanNow)
	...
	local statuses = nil
	if scanNow then
		statuses = ScanUnitStatuses(member)
	else
		local cached = statusCache[member.key]
...
RefreshBuffObserver = function(scanNow)
	...
	if member == nil then
		buffObserverMemberLabel:SetText("观察对象：当前没有可读取成员")
		buffObserverVisibleStatuses = {}
		for row = 1, buffObserverPageSize do buffObserverRows[row].label:SetText("") buffObserverRows[row].add:Show(false) end
		buffObserverPageLabel:SetText("0 个状态")
		return
	end
	buffObserverMemberLabel:SetText(string.format("观察对象：%s · %d团 #%d", tostring(member.name), tonumber(member.raidIndex) or 1, tonumber(member.memberIndex) or 1))
	...
	for row = 1, buffObserverPageSize do
		...
		if status ~= nil then
			...
			item.label:SetText(string.format("%s  [%s]  ID:%s", tostring(status.name or status.id), StatusSourceText(status), tostring(status.id)))
			item.add:SetText(tracked and "已追加" or "追加")
			item.add:Show(true)
		else
			item.label:SetText("")
			item.add:Show(false)
	...
	buffObserverPageLabel:SetText(string.format("第 %d/%d 页 · %d 个状态", page, pages, #statuses))
end
...
local function HasViewportChanged()
...
-- Internal phase timings exist only during the operator-started detailed
-- capture.  Normal runtime keeps one coarse OnUpdate label and adds no
-- per-phase bookkeeping to the already hot healing path.
local function ProfileHealerPhase(label, callback, argument)
	local monitor = SuitePerformance
	if monitor == nil or monitor.capture == nil or monitor.capture.finished == true then return callback(argument) end
...
local function RunHealerRuntimeTick(dt)
	...
	animationClock = animationClock + deltaMs
	healthElapsed = healthElapsed + deltaMs
	buffElapsed = buffElapsed + deltaMs
	rosterElapsed = rosterElapsed + deltaMs
	visualElapsed = visualElapsed + deltaMs
	metricsElapsed = metricsElapsed + deltaMs
	buffObserverScanElapsed = (buffObserverScanElapsed or 0) + deltaMs
	buffObserverRosterElapsed = (buffObserverRosterElapsed or 0) + deltaMs

	teamRosterSettleRemainingMs = math.max(
		0,
		(tonumber(teamRosterSettleRemainingMs) or 0) - deltaMs
	)
	local teamRosterStable = teamRosterSettleRemainingMs <= 0

	if state.enabled and teamRosterStable then
		if rosterElapsed >= 1000 then
			rosterElapsed = 0
			ProfileHealerPhase("healer:roster_rebuild", RebuildRoster)
		end
		if buffElapsed >= state.buffScanMs then
			buffElapsed = 0
			ProfileHealerPhase("healer:status_scan", ScanAllStatuses)
		end
		if healthElapsed >= state.healthScanMs then
			healthElapsed = 0
			ProfileHealerPhase("healer:health_recommendations", ScanHealthAndBuildRecommendations)
		end
	end
	if teamRosterStable and buffObserverWindow ~= nil and buffObserverWindow:IsVisible() then
		if not state.enabled and buffObserverRosterElapsed >= 1000 then
			buffObserverRosterElapsed = 0
			ProfileHealerPhase("healer:observer_roster", RebuildRoster)
		end
		if buffObserverScanElapsed >= math.max(300, state.buffScanMs) then
			buffObserverScanElapsed = 0
			-- When healer runtime is enabled, the Authority scan already refreshed
			-- the whole roster; reuse that cache. When disabled, scan only the one
			-- member currently selected in the observer.
			ProfileHealerPhase("healer:observer_refresh", RefreshBuffObserver, not state.enabled)
		end
	end

	if visualElapsed >= 50 then
		visualElapsed = 0
		ProfileHealerPhase("healer:visual_recommendations", RefreshRecommendationPanel)
		ProfileHealerPhase("healer:head_markers", RefreshHeadMarkers)
		ProfileHealerPhase("healer:raid_highlights", RefreshRaidHighlights)
		-- Reassert overlay ordering only after the native raid frame settle window.
		-- This replaces the old TEAM_MEMBERS_CHANGED callback-side Raise calls.
		if teamRosterStable and teamRosterZOrderPending == true then
			teamRosterZOrderPending = false
			if EnsureRaidOverlayZOrder ~= nil then
				for sectionIndex = 1, 4 do
					local overlay = raidOverlays[sectionIndex]
					local visible = overlay ~= nil and overlay.window ~= nil and overlay.window:IsVisible()
					EnsureRaidOverlayZOrder(overlay, visible == true)
				end
			end
		end
	end
	if metricsElapsed >= 500 then
		metricsElapsed = 0
		if HasViewportChanged() then
			ProfileHealerPhase("healer:layout", LayoutAll)
		end
		if configWindow ~= nil and configWindow:IsVisible() then
			ProfileHealerPhase("healer:settings_refresh", RefreshSettingsUi)
		end
	end
end
...
function HM:DisableRuntime()
    ...
    for _, marker in pairs(headMarkers or {}) do
        if marker.window ~= nil then marker.window:Show(false) end
    end
    for _, overlay in pairs(raidOverlays or {}) do
        if overlay.window ~= nil then overlay.window:Show(false) end
    end
    return true
end
...
local SUITE_SETTING_KEYS = {
    maxDistance=true, enterThreshold=true, exitThreshold=true, selfThreshold=true,
    emergencyThreshold=true, lowHealthThreshold=true, proximityMode=true, proximityDistance=true,
    headMarkerCount=true, showHeadName=true, showHeadDistance=true, showHeadScore=true,
    showRaidRanks=true, raidRankCount=true, raidRankFontSize=true, raidRankAlpha=true,
    raidRankCorner=true, raidRankOffsetX=true, raidRankOffsetY=true,
    healthScanMs=true, buffScanMs=true, minHoldMs=true, scoreLead=true,
    healthCurveMode=true, healthAccelMode=true, distanceCurveMode=true, distanceEdgePercent=true,
    missingSensitivity=true, raidEffectMode=true, headEffectMode=true, headShapeMode=true,
    roleScoringEnabled=true, raidCalibrationSection=true, raidCalibrationScope=true,
    manualRaidPage=true, manualRaidPageLocked=true,
}
local SUITE_BOOLEAN_KEYS = {
    proximityMode=true, showHeadName=true, showHeadDistance=true, showHeadScore=true,
    showRaidRanks=true, roleScoringEnabled=true, manualRaidPageLocked=true,
}
local SUITE_NUMBER_LIMITS = {
    maxDistance={1,100}, enterThreshold={1,100}, exitThreshold={1,100}, selfThreshold={1,100},
    emergencyThreshold={1,100}, lowHealthThreshold={1,100}, proximityDistance={1,100}, headMarkerCount={0,20},
    raidRankCount={0,50}, raidRankFontSize={8,20}, raidRankAlpha={0.10,1.00}, raidRankCorner={1,4},
    raidRankOffsetX={-50,50}, raidRankOffsetY={-50,50}, healthScanMs={100,1000}, buffScanMs={200,2000},
    minHoldMs={0,5000}, scoreLead={0,50}, healthCurveMode={1,4}, healthAccelMode={1,4},
    distanceCurveMode={1,4}, distanceEdgePercent={0,100}, missingSensitivity={0,120000},
    raidEffectMode={1,4}, headEffectMode={1,4}, headShapeMode={1,8}, raidCalibrationSection={1,4}, raidCalibrationScope={1,3},
    manualRaidPage={1,2},
}
...
function HM:GetSuiteSetting(key)
    key=tostring(key or "")
    if SUITE_SETTING_KEYS[key]~=true then return nil end
    return state[key]
end
function HM:SetSuiteSetting(key, value)
    key=tostring(key or "")
    if SUITE_SETTING_KEYS[key]~=true then return false, "unsupported setting" end
    local limits=SUITE_NUMBER_LIMITS[key]
    if limits~=nil then
        local number=tonumber(value)
        if number==nil then return false, "numeric setting required" end
        value=Clamp(number,limits[1],limits[2])
        if key=="raidRankCorner" or key=="healthCurveMode" or key=="healthAccelMode" or key=="distanceCurveMode"
            or key=="raidEffectMode" or key=="headEffectMode" or key=="headShapeMode"
            or key=="raidCalibrationSection" or key=="raidCalibrationScope" or key=="manualRaidPage" then
            value=math.floor(value + 0.5)
        end
    elseif SUITE_BOOLEAN_KEYS[key]==true then
        value=value==true
    end
    if key=="enterThreshold" then
        value=Clamp(value,1,tonumber(state.exitThreshold) or 100)
    elseif key=="exitThreshold" then
        value=Clamp(value,tonumber(state.enterThreshold) or 1,100)
    end
    state[key]=value
...
local SUITE_COLOR_KEYS = {
    proximityColor=true, lowHealthColor=true, emergencyColor=true,
}
...
function HM:GetSuiteHeadSize(level)
    ...
    return math.floor(Clamp(tonumber(state.headSizes[level]) or tonumber(defaults.headSizes[level]) or 18,12,60))
end
function HM:SetSuiteHeadSize(level,value)
    level=math.floor(Clamp(tonumber(level) or 1,1,4))
    state.headSizes=type(state.headSizes)=="table" and state.headSizes or DeepCopy(defaults.headSizes)
    state.headSizes[level]=math.floor(Clamp(tonumber(value) or state.headSizes[level] or defaults.headSizes[level] or 18,12,60)+0.5)
    RefreshHeadMarkers()
    SaveState()
    return true
end
...
function HM:SetSuiteColorChannel(key, channel, value)
    ...
    if channel~="r" and channel~="g" and channel~="b" and channel~="a" then return false, "unsupported channel" end
    state[key][channel]=Clamp(tonumber(value) or state[key][channel] or 0, channel=="a" and 0.05 or 0, 1)
...
local SUITE_WEIGHT_KEYS={health=true,distance=true,missing=true,unprotected=true}
...
local SUITE_LEVEL_KEYS={attention=true,high=true,emergency=true}
...
function HM:SetSuiteLevelThreshold(key,value)
    ...
    value=tonumber(value) or state.levelThresholds[key] or 1
    if key=="attention" then value=Clamp(value,1,(tonumber(state.levelThresholds.high) or 60)-1)
    elseif key=="high" then value=Clamp(value,(tonumber(state.levelThresholds.attention) or 40)+1,(tonumber(state.levelThresholds.emergency) or 80)-1)
    else value=Clamp(value,(tonumber(state.levelThresholds.high) or 60)+1,100) end
    state.levelThresholds[key]=math.floor(value+0.5)
    RefreshSuiteProjection(); SaveState(); return true
end
...
function HM:SetSuiteLevelColorChannel(level,channel,value)
    ...
    state.levelColors[level][channel]=Clamp(tonumber(value) or state.levelColors[level][channel] or 0,channel=="a" and 0.05 or 0,1)
    RefreshSuiteProjection(); SaveState(); return true
end
...
local SUITE_ROLE_SCORE_KEYS={normal=true,mainTank=true,offTank=true,healer=true,unknown=true}
...
function HM:SetSuiteRoleOverride(name,role)
    ...
    state.roleOverrides[name]=role; SaveState(); return true
end
function HM:RemoveSuiteRoleOverride(name)
    ...
    state.roleOverrides[name]=nil; SaveState(); return true
end
...
function HM:GetSuiteObservedStatuses(memberIndex)
    ...
    local statuses=type(ScanUnitStatuses)=="function" and ScanUnitStatuses(member) or {}
    local out={}
...
function HM:SetTrackedBuffColorChannel(index, channel, value)
    ...
    if channel~="r" and channel~="g" and channel~="b" and channel~="a" then return false,"unsupported channel" end
    entry.color[channel]=Clamp(tonumber(value) or entry.color[channel] or 0, channel=="a" and 0.05 or 0, 1)
    RefreshSuiteProjection(); SaveState(); return true
end
...
local SUITE_RULE_KEYS={
    purpose=true,sourceMode=true,matchMode=true,minStacks=true,minRemainingMs=true,unknownRemainingValid=true,
    healthRangeEnabled=true,healthMin=true,healthMax=true,effectType=true,scoreMode=true,scoreValue=true,
    allowStack=true,emergencyRetainPercent=true,countsAsProtection=true,displayPriority=true,rescuePriority=true,
    distanceMode=true,customDistance=true,healPriorityThreshold=true,excludeDisplayMode=true,
}
local SUITE_RULE_BOOLEAN={unknownRemainingValid=true,healthRangeEnabled=true,allowStack=true,countsAsProtection=true}
local SUITE_RULE_LIMITS={
    purpose={1,5},sourceMode={1,5},matchMode={1,4},minStacks={1,99},minRemainingMs={0,120000},
    healthMin={0,100},healthMax={0,100},effectType={1,5},scoreMode={1,4},scoreValue={-100,100},
    emergencyRetainPercent={0,100},displayPriority={0,200},rescuePriority={0,200},distanceMode={1,3},
    customDistance={1,100},healPriorityThreshold={0,100},excludeDisplayMode={1,3},
}
...
function HM:GetSuiteRuleSetting(index,key)
    ...
    if r==nil or SUITE_RULE_KEYS[key]~=true then return nil end
    return r[key]
end
function HM:SetSuiteRuleSetting(index,key,value)
    local r=state.rules and state.rules[tonumber(index) or 0]; key=tostring(key or "")
    if r==nil then return false,"规则不存在" end
    if SUITE_RULE_KEYS[key]~=true then return false,"unsupported rule setting" end
    if SUITE_RULE_BOOLEAN[key]==true then value=value==true else
        local lim=SUITE_RULE_LIMITS[key]; if lim then value=Clamp(tonumber(value) or r[key] or lim[1],lim[1],lim[2]) end
    end
    if key=="healthMin" then value=math.min(value,tonumber(r.healthMax) or 100) end
    if key=="healthMax" then value=math.max(value,tonumber(r.healthMin) or 0) end
    r[key]=value; SaveState(); return true
end
...
function HM:SetSuiteRuleColorChannel(index,channel,value)
    local r=state.rules and state.rules[tonumber(index) or 0]; if r==nil then return false,"规则不存在" end
    channel=tostring(channel or ""); if channel~="r" and channel~="g" and channel~="b" and channel~="a" then return false,"unsupported channel" end
    r.color=CopyColor(r.color,{r=0.72,g=0.30,b=1.00,a=0.82})
    r.color[channel]=Clamp(tonumber(value) or r.color[channel] or 0,channel=="a" and 0.05 or 0,1)
    SaveState(); return true
end
...
function HM:SaveSuiteSettings() SaveState(); return true end
