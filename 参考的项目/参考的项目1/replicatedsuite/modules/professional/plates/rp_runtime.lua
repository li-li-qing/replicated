...
------------------------------------------------------------------------
-- Replicated Plates - Single runtime host (v0.5.0 BUFF UI2 + strict Hidden whitelist)
...
local IMPORTANT_COOLDOWN_LIBRARY = {
    -- Glider / wings: common mobility and combat actives.
    ...
    -- Combat-relevant Groa / Powerstone Pet actives. All durations below are
    -- discovery/unit hints only; the visible timer still comes from live
    -- X2Skill cooldown APIs and never from a locally simulated countdown.
...
function R:OnUpdate(dt)
    ...
    if self.forceGlobal or self.positionAccumulator >= self.positionIntervalMs then
    ...
    if self.forceGlobal or self.metadataAccumulator >= self.metadataIntervalMs then
    ...
    if self.forceGlobal or self.targetMetaAccumulator >= self.targetMetaIntervalMs then
        self.targetMetaAccumulator = 0
        self:SafeLane("target_extras", "target", R.UpdateTargetExtras)
    end
    if self.forceGlobal or self.equipmentDirty == true then self:SafeLane("equipment", "player", R.UpdateEquipment) end
    if self.forceGlobal or self.cooldownAccumulator >= self.cooldownIntervalMs then
        self.cooldownAccumulator = 0
        self:SafeLane("important_cooldowns", "player", R.UpdateImportantCooldowns)
    end
    if self.forceGlobal or self.healthAccumulator >= self.healthIntervalMs then
        self.healthAccumulator = 0
        for _, scope in ipairs(SCOPE_ORDER) do self:SafeLane("health:" .. scope, scope, R.UpdateHealth, scope) end
    end
    if self.forceGlobal or self.distanceAccumulator >= self.distanceIntervalMs then
    ...
    for _, scope in ipairs(SCOPE_ORDER) do
        local st = self.scopes[scope]
        if self.forceGlobal or st.force or st.effectsAccumulator >= EFFECT_INTERVAL[scope] then
            st.effectsAccumulator = 0
            self:SafeLane("effects:" .. scope, scope, R.UpdateEffects, scope)
        end
    end
    if self.forceGlobal or self.castAccumulator >= self.castIntervalMs then
        self.castAccumulator = 0
        self:SafeLane("casting", "target", R.UpdateCasting)
    end

    -- Explicit status-capture mode is opt-in from the tracking page.  Its
    -- periodic lane is ID-only; expensive metadata decoding is event-driven.
    if self.captureAccumulator >= self.captureIntervalMs then
    ...
    -- Buff-count cap warning lane (report 八-P0-1). 1s cadence; the lane
    -- short-circuits before any API read when disabled.
    if self.forceGlobal or self.buffcapAccumulator >= self.buffcapIntervalMs then
        self.buffcapAccumulator = 0
        self:SafeLane("buffcap", nil, R.UpdateBuffCap)
    end

    -- Magic-circle distance lane (report 八-P1-1). 200ms cadence; the lane
    -- short-circuits before any API read when disabled, and only reads the
    -- player world position after a buff hit.
    if self.forceGlobal or self.magicCircleAccumulator >= self.magicCircleIntervalMs then
        self.magicCircleAccumulator = 0
        self:SafeLane("magiccircle", nil, R.UpdateMagicCircle)
    end

    -- watchtarget aggro/distance windows (report 七-C). 200ms cadence; the
    -- lane short-circuits before any read when both windows are off.
    if self.forceGlobal or self.watchAccumulator >= self.watchIntervalMs then
        self.watchAccumulator = 0
        self:SafeLane("watchtarget", nil, R.UpdateWatchWindows)
    end

    -- Combat alerts (report 七-方案A). 200ms cadence; the lane short-circuits
    -- before any read when alerts.enabled is off.
    if self.forceGlobal or self.alertsAccumulator >= self.alertsIntervalMs then
        self.alertsAccumulator = 0
        self:SafeLane("alerts", nil, R.UpdateAlerts)
    end

    -- Unit connection lines (report 七-方案B). Cadence is configurable
    -- (lines.updateMs, Optimize 1); disabled or no enabled pair short-circuits
    -- to zero API reads.
    ...
    if self.forceGlobal or self.linesAccumulator >= linesMs then
        self.linesAccumulator = 0
        self:SafeLane("lines", nil, R.UpdateLines)
    end

    -- Player-centred distance circle (F5). 100ms cadence; disabled -> zero reads.
    if self.forceGlobal or self.circleAccumulator >= self.circleIntervalMs then
        self.circleAccumulator = 0
        self:SafeLane("circle", nil, R.UpdateCircle)
    end

    -- Session discovery runs from this same host even when the manager is closed.
    -- It records only ids/metadata in memory and never auto-tracks or writes SaveData.
    if self.discoveryAccumulator >= self.discoveryIntervalMs then
    ...
    if self.managerAccumulator >= self.managerIntervalMs then
        ...
        if P.Manager ~= nil and type(P.Manager.RuntimeRefresh) == "function" then
            ...
            if managerOk then
                self.laneErrors.manager = nil
            else
                local text = tostring(managerErr or "unknown")
                if self.laneErrors.manager ~= text then
                    self.laneErrors.manager = text
                end
            end
        end
    end

    -- A lane that failed this pass stays forced for the next pass. Previously
    -- all force flags were cleared unconditionally, which could strand a scope
    -- after a persistent client/UI exception.
    for _, scope in ipairs(SCOPE_ORDER) do
...
local function InstallRuntimeHandlers()
    local function Bind(widget, handlerName, handler)
        local shared = rawget(_G, "ReplicatedSuiteShared")
        local native = shared and shared.NativeSafe or nil
    ...
    if R.driver ~= nil and type(R.driver.SetHandler) == "function" then
        ReleaseWidgetHandler(R.driver, "OnEvent")
        ReleaseWidgetHandler(R.driver, "OnUpdate")
        local eventOk, eventErr = Bind(R.driver, "OnEvent", RuntimeEventHandler)
        if not eventOk then ReleaseRuntimeHandlers(); return false, eventErr end
        local updateOk, updateErr = Bind(R.driver, "OnUpdate", RuntimeUpdateHandler)
    ...
    if R.watchdog ~= nil and type(R.watchdog.SetHandler) == "function" then
        ReleaseWidgetHandler(R.watchdog, "OnUpdate")
        local watchdogOk, watchdogErr = Bind(R.watchdog, "OnUpdate", WatchdogUpdateHandler)
    ...
    if currentHeartbeat == watchdogObservedHeartbeat then
        ...
        if R.driver ~= nil then
            ...
            if type(R.driver.SetHandler) == "function" then
                ReleaseWidgetHandler(R.driver, "OnUpdate")
                pcall(R.driver.SetHandler, R.driver, "OnUpdate", RuntimeUpdateHandler)
            end
        end
        R:ForceAll()
        local recoveryBefore = tonumber(R.heartbeatSerial) or 0
        local pass, err = pcall(R.OnUpdate, R, 16)
        local recoveryAfter = tonumber(R.heartbeatSerial) or 0
        if pass and recoveryAfter > recoveryBefore then
            R.successfulUpdateSerial = (tonumber(R.successfulUpdateSerial) or 0) + 1
            currentHeartbeat = recoveryAfter
            R.laneErrors.watchdog = nil
        elseif pass then
            local text = "Runtime recovery pass made no heartbeat progress"
