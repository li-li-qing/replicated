...
local function NormalizeEffect(effectType, index, extra, tip)
    ...
    -- extendedplates carries one server-specific correction for Defiance
    -- hidden buff 22969: its exposed timer includes an extra 24 minutes.
    if effectType == "hidden" and idText == "22969" then
...
-- Runtime scan. When trackedOnly is enabled, non-matching rows are rejected
-- using the cheap effect id from UnitBuff/UnitDeBuff before tooltip decoding.
-- Id-only effect scan (report 七-方案A alerts consumer). Cheap: reads only
-- the count + row data ids, never tooltips. Returns { id, ... } list.
function A:GetEffectIds(unit, effectType)
    local methods = EFFECT_METHODS[effectType]
    if methods == nil or X2Unit == nil or type(X2Unit[methods.count]) ~= "function"
        or type(X2Unit[methods.data]) ~= "function" then return {} end
    ...
    for index = 1, count do
        ...
        if rawId ~= nil and tostring(rawId) ~= "" then
            result[#result + 1] = tostring(rawId)
        end
    end
    return result
end
...
-- Cheap rolling id scan used by V4 categorized session discovery.  It prefers the raw
-- UnitBuff/UnitDeBuff row id and only decodes a tooltip when that client row
-- does not expose an id.  The circular cursor means even very large aura sets
-- are covered over several ticks without a full deep scan every frame.
function A:GetEffectIds(unit, effectType, scanLimit, startIndex, tooltipFallbackLimit)
    ...
    local cursor = math.floor(tonumber(startIndex) or 1)
...
local TARGET_ARMOR_SET_BY_BUFF = {
    [16551] = "布甲", [16552] = "皮甲", [16553] = "板甲",
    -- Older complete-set ids are retained as compatibility fallbacks.
    [714] = "布甲", [716] = "皮甲", [740] = "板甲",
}
local TARGET_WEAPON_STYLE_BY_BUFF = {
    [16557] = "双手", [16558] = "双持", [16559] = "盾牌",
    -- Legacy equip-state ids still emitted by current RU clients.
